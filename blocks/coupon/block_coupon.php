...
class block_coupon extends block_base {
    ...
    public function get_content() {
        ...
        if (has_capability('block/coupon:generatecoupons', $this->context)) {
            ...
            if($access >= MANAGER_ACCESS) $menuitems[] = html_writer::link($urlmanagelogos, get_string('url:managelogos', 'block_coupon'));
            ...
            if($access >= MANAGER_ACCESS) $menuitems[] = html_writer::link($requestusersurl, get_string('tab:requestusers', 'block_coupon'));
            $requestsurl = new \moodle_url($CFG->wwwroot . '/blocks/coupon/view/requests/admin.php', $baseparams + ['action' => 'requests']);
            if($access >= MANAGER_ACCESS) $menuitems[] = html_writer::link($requestsurl, get_string('tab:requests', 'block_coupon'));
        }
        ...
        if (has_capability('block/coupon:viewreports', $this->context)) {
            ...
            if($access >= MANAGER_ACCESS) $menuitems[] = html_writer::link($urlreports, get_string('url:view_reports', 'block_coupon'));
