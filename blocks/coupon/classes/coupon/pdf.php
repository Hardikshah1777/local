...
class pdf extends \pdf {
    ...
    protected function write_coupon_pages() {
        foreach ($this->coupons as $coupon) {
			...
			} elseif (!empty($dim = helper::get_dimensions($coupon->logoid))){
                $txtmain = '<div style="text-align: center; font-size: 200%; font-weight: bold">'.$coupon->submission_code.'</div>';
                $this->MultiCell(190,150,$txtmain,false, 'C', false, 1, $dim['x'], $dim['y'], true, 0, true);
            } else {
                $this->MultiCell(190, 150, $txtmain, false, 'C', false, 1, 15, 75, true, 0, true);
            }
			...
			if(helper::print_courses($coupon->logoid)){
                $this->SetTextColorArray([255,255,255]);
                $this->MultiCell($this->getPageWidth() - 24,10,'<h1>'.implode(',',$courses).'</h1>',false,'C',false,1,12,80,true,0,true);
            }
			if(helper::print_courses_yyc($coupon->logoid)){
                $this->SetTextColorArray([255,255,255]);
                $this->MultiCell($this->getPageWidth() - 24,10,'<h1 style="font-size:50px;">'.implode(',',$courses).'</h1>',false, 'C',false,1,12,38,true,0,true);
            }
