...
class helper {
    ...
    public static final function mail_coupons($coupons, $emailto, $generatesinglepdfs = false,
            $emailbody = false, $initiatedbycron = true,$appendtoemail = false) {
        global $DB, $CFG, $USER;
        raise_memory_limit(MEMORY_HUGE);
        $phpmailer = self::generate_coupon_mail($emailto, $emailbody, $initiatedbycron);

        $courseids = $DB->get_records_list('block_coupon_courses','couponid',array_column($coupons,'id'));
        $courses = $DB->get_records_list('course','id',array_column($courseids,'courseid'));
        $courseshtml = join(' , ',array_column($courses,'fullname'));

        if(strpos($phpmailer->Body,'##courses##') !== false)
            $phpmailer->Body = str_replace('##courses##',$courseshtml,$phpmailer->Body);
        else
            $phpmailer->Body .= $courseshtml;

        $couponshtml = join(' , ',array_column($coupons,'submission_code'));
        if(strpos($phpmailer->Body,'##coupons##') !== false)
            $phpmailer->Body = str_replace('##coupons##',$couponshtml,$phpmailer->Body);
        else
        $phpmailer->Body .= $couponshtml;
		...
		$owner = \core_user::get_user($coupon->ownerid);
		/*if(!is_siteadmin()) */$phpmailer->addCC($owner->email);
    ...
    public static function get_price($groupid = 0){
        global $DB;
        $default_price = 80;
        if(empty($groupid)) return $default_price;
        $price = $DB->get_field('block_coupon_group_pricing','price',array('groupid' => $groupid));
        if (empty($price)) $price = $default_price;
        return $price;
    }
    ...
    public static function get_default_logoid(){
        return self::$logoid1;
    }

	public static function get_access(){
        !defined('MANAGER_ROLEID') && define('MANAGER_ROLEID',10);
        !defined('ADMIN_ACCESS') && define('ADMIN_ACCESS',10);
        !defined('MANAGER_ACCESS') && define('MANAGER_ACCESS',8);
        !defined('NO_ACCESS') && define('NO_ACCESS',0);
        if(is_siteadmin()) return ADMIN_ACCESS;
        $user_roles = get_user_roles(\context_system::instance());
        $user_role_ids = array_column($user_roles,'roleid');
        if(in_array(MANAGER_ROLEID,$user_role_ids)) return MANAGER_ACCESS;
        return NO_ACCESS;
    }
    private static $logoid1 = 8571;
    private static $logoid2 = 11026;
	private static $logoid3 = 14550; /*old 14356*/
	private static $logoid4 = 14719;
    public static function get_dimensions($logoid){
        switch ($logoid){
            case self::$logoid1:
                $dimension = array(
                    'x' => 15,
	...
	public static function get_course_logoid($courseid){
        $logomap = array(
            4 => self::$logoid2,
        );
        return array_key_exists($courseid,$logomap)?$logomap[$courseid]:self::get_default_logoid();
    }
	public static function print_courses($logoid){
	    return $logoid == self::$logoid3;
    }
	public static function print_courses_yyc($logoid){
	    return $logoid == self::$logoid4;
    }
}
