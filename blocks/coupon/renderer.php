...
class block_coupon_renderer extends plugin_renderer_base {
    ...
    public function get_tabs($context, $selected, $params = array()) {
        ...
        $tabs[] = $this->create_pictab('cpgrouppricing', 'i/group', '',
                new \moodle_url($CFG->wwwroot . '/blocks/coupon/view/group_pricing.php',
                        array_merge($params, array())),
                get_string('tab:cpgrouppricing', 'block_coupon'));
        $tabs[] = $this->create_pictab('cppurchasehistory', 'i/report', '',
                new \moodle_url($CFG->wwwroot . '/blocks/coupon/view/purchase_history.php',
                        array_merge($params, array())),
                get_string('tab:cppurchasehistory', 'block_coupon'));

        $access = \block_coupon\helper::get_access();
        /*SSEE*/if($access < MANAGER_ACCESS) unset($tabs[1],$tabs[2],$tabs[3],$tabs[6],$tabs[8],$tabs[9]);
        /*SSEE*/if($access < ADMIN_ACCESS) unset($tabs[7]);
    ...
    public function page_group_pricing($id,$courseid,$groupid) {
        global $DB;
        $dbtable = 'block_coupon_group_pricing';
        $table = new \block_coupon\tables\group_pricing($id,$courseid,$groupid);
        $form = new \block_coupon\forms\coupon\group_pricing(null,array('id'=>$id,'courseid'=>$courseid,'groupid'=>$groupid));
        $redirecturl = new \moodle_url($this->page->url->out_omit_querystring(),array('id'=>$id,'courseid'=>$courseid));
        $params = array('courseid'=>$courseid,'groupid'=>$groupid);
        if(!$record = $DB->get_record($dbtable,$params)){
        ...
        if($data = $form->get_data()){
            ...
            if(!empty($record->id)){
                ...
                $DB->insert_record($dbtable,$record);
            }
            redirect($redirecturl,get_string('page:group_pricing:redirectmsg','block_coupon'));
        ...
        $out .= html_writer::start_div('block-coupon-container');
        $out .= html_writer::start_div();
        $out .= $this->get_tabs($this->page->context, 'cpgrouppricing',array('id'=>$id));
        $out .= html_writer::end_div();
        if(!empty($groupid)){
            $out .= $form->render();
        }else {
    ...
    protected function get_coupon_generator_paypal_page($mform) {
        $out = '';
        $out .= $this->header();
        $out .= html_writer::start_div('block-coupon-container');
        $out .= html_writer::start_div();
    ...
    public function get_paypal_data($groupid = 0) {
        global $USER, $DB, $CFG;
        $data = array();
        $price = \block_coupon\helper::get_price($groupid);
        $url = new \moodle_url('/blocks/coupon/ipn.php');

        $data['cmd'] = '_xclick';
        $data['business'] = get_config('block_coupon','coupon_paypalemail');
        $data['item_number'] = 0;
        $data['quantity'] = '1';
        $data['userid'] = $USER->id;#userid
        $data['groupid'] = $groupid;#groupid
        $data['blockid'] = $this->page->url->param('id');#blockid
        $data['currency_code'] = \block_coupon\helper::get_currency_code();
        $data['baseval'] = $price;
        $data['for_auction'] = 'false';
        $data['no_note'] = '1';
        $data['no_shipping'] = '1';
        $data['notify_url'] = $url->out();
        $data['return'] = $url->out();
        $data['cancel_return'] = $this->page->url->out();
        $data['rm'] = '2';
    ...
    public function page_coupon_generator_paypal() {
        global $CFG, $USER, $DB;
        $out = '';
        $courseprices = $course_options = array();
        $price = \block_coupon\helper::get_price();
        $courses = $DB->get_records_sql('SELECT 
                c.id,g.id as groupid,c.fullname as coursename,COALESCE(gp.price,'.$price.') AS price 
                FROM {groups} g 
                JOIN {groups_members} gm ON gm.groupid = g.id
                JOIN {course} c ON c.id = g.courseid AND c.visible = 1
                LEFT JOIN {block_coupon_group_pricing} gp ON gp.groupid = g.id AND gp.courseid = g.courseid
                WHERE gm.userid = :userid ORDER BY gm.timeadded ASC', array('userid' => $USER->id));

        foreach ($courses as $course){
            $course_options[$course->id] = $course->coursename;
            $courseprices[$course->id] = array('g'=>$course->groupid,'p' => $course->price);
        }

        $use_sandbox = get_config('block_coupon','coupon_usepaypalsandbox');
        $paypalurl = 'https://www.'.(empty($use_sandbox) ? '' :'sandbox.').'paypal.com/cgi-bin/webscr';

        $data = $this->get_paypal_data();
        if(!empty($CFG->bypasspaypal)) $paypalurl = $data['return'];

        $table = new \block_coupon\tables\paypal();
        $table->baseurl = $this->page->url;
        $mform = new block_coupon\forms\coupon\generator_paypal($paypalurl, array('course_options' => $course_options,'courseprices'=>json_encode($courseprices)));
        $mform->set_data($data);

        $out .= $this->get_coupon_form_page_table($mform,$table);

        return $out;
    }

    protected function get_coupon_form_page_table($mform,$table) {
        $out = '';
        $out .= $this->header();
        $out .= html_writer::start_div('block-coupon-container');
        $out .= html_writer::start_div();
    ...
    public function page_coupon_generator_email($record) {
        global $CFG,$DB,$USER;
        $out = '';
        $customdata = array(
        ...
        $mform = new block_coupon\forms\coupon\generator_email(null, $customdata);
        $redirect_url = new moodle_url('/blocks/coupon/view/generate_coupon.php',array('id'=>$this->page->url->param('id')));
        if($data = $mform->get_data()){
            $generatoroptions = generatoroptions::from_session();
            $generatoroptions->ownerid = $record->userid;
            $generatoroptions->type = generatoroptions::COURSE;
			$generatoroptions->logoid = \block_coupon\helper::get_course_logoid($record->courseid);
            $courseid = $record->courseid;
            $generatoroptions->courses = (array)$courseid;

            $generatoroptions->groups = (array)$record->groupid;
            $generatoroptions->senddate = time();
            $bodyparams = array(
                'to_name' => fullname($USER),
                'from_name' => fullname($USER),
            );
            $generatoroptions->emailbody = get_string('coupon_mail_content', 'block_coupon', $bodyparams);

            $generatoroptions->recipients = $mform->get_recipients_from_csv($data->coupon_recipients_manual);

            $generatoroptions->redirecturl = $CFG->wwwroot . '/my';
            $generatoroptions->enrolperiod = null;
            ...
            $coupons = $DB->get_records_list('block_coupon', 'id', $generator->get_generated_couponids());
            foreach($coupons as $coupon) helper::mail_coupons(array($coupon),$coupon->for_user_email,false);

            $record->used  = $record->used + count($generatoroptions->recipients);
            $DB->update_record('block_coupon_purchase_info',$record);
            redirect($redirect_url,get_string('page:generate_email:successmsg','block_coupon'));
     ...
     * @return string
     */
    public function page_coupon_generator_singlestep($isadmin = false) {
        global $USER, $CFG,$DB;
        // Create form.
        $mform = new block_coupon\forms\coupon\generator_singlestep($this->page->url,array('isadmin'=>$isadmin));
        ...
        if ($mform->is_cancelled()) {
            ...
            $generatoroptions->ownerid = $USER->id;
            $generatoroptions->type = generatoroptions::COURSE;
            $generatoroptions->logoid = isset($data->logo)?$data->logo:\block_coupon\helper::get_default_logoid();;//default logo put id here
            $courseid = !empty($data->coupon_courses)?$data->coupon_courses:2;
            $generatoroptions->courses = (array)$courseid;

            $defalt_role = helper::get_default_coupon_role()->id;
            $generatoroptions->roleid = !empty($defalt_role)?$defalt_role:5;
            if (isset($data->coupon_groups) && !empty($data->coupon_groups)) {
                $generatoroptions->groups = (array)$data->coupon_groups;
            }else if(!$isadmin){
                $groups = reset(groups_get_user_groups($courseid));
                $generatoroptions->groups = isset($groups[0])?(array)$groups[0]:null;
            }
            if($isadmin){
                if ($data->showform == 'csv') {
                    $generatoroptions->senddate = time();
                    $generatoroptions->recipients = $mform->get_recipients_from_csvfile($mform->get_file_content('coupon_recipients'));
                    $generatoroptions->emailbody = $data->email_body['text'];
                }
            ...
            $generatoroptions->enrolperiod = null;
            $generatoroptions->renderqrcode = (isset($data->renderqrcode) && $data->renderqrcode) ? true : false;
            //die(print_object($generatoroptions));

            $generator = new generator();
            //print_object($generatoroptions);die;
            $result = $generator->generate_coupons($generatoroptions);
            if ($result !== true) {
                // Means we've got an error.
                // Don't know yet what we're gonne do in this situation. Maybe mail to supportuser?
            ...
            $coupons = $DB->get_records_list('block_coupon', 'id', $generator->get_generated_couponids());
            if (!$isadmin || $data->showform == 'amount') {
                foreach ($coupons as $coupon) {
                    $coupon->for_user_email = $data->alternative_email;
                    $DB->update_record('block_coupon', $coupon);
                }

                helper::mail_coupons($coupons, $generatoroptions->emailto, $generatoroptions->generatesinglepdfs);
                //if(!$isadmin) helper::mail_coupons($coupons, $USER->email, $generatoroptions->generatesinglepdfs);
            }
            redirect(new moodle_url($CFG->wwwroot . '/my'), get_string('coupons_sent', 'block_coupon'));
        }
    ...
    public function page_purchase_history(){
        $table = new \block_coupon\tables\purchase();
        $table->baseurl = $this->page->url;
        $out = '';
        $out .= $this->header();
        $out .= html_writer::start_div('block-coupon-container');
        $out .= html_writer::start_div();
        $out .= $this->get_tabs($this->page->context, 'cppurchasehistory', array('id' => $this->page->url->param('id')));
        $out .= html_writer::end_div();
        $out .= $table->render();
        $out .= html_writer::end_div();
        $out .= $this->footer();
