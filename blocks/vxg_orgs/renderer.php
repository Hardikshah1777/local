...
class block_vxg_orgs_renderer extends plugin_renderer_base
{
    ...
    protected function edit_buttons($returnurl)
    {
        ...
        if (has_capability('block/vxg_orgs:manageorgadmins', context_system::instance())) {
            $addadminurl = new moodle_url('/blocks/vxg_orgs/manage_admins.php', ['returnurl' => $returnurl]);
            ...
            $viewadminurl = new moodle_url('/blocks/vxg_orgs/viewadmin.php', ['returnurl' => $returnurl]);
            $viewadminbtn = html_writer::link($viewadminurl, html_writer::tag('button', '<span class="icon fa fa-eye fa-fw"></span>',
                array('class' => 'btn btn-info', 'data-toggle' => 'tooltip', 'title' => get_string('viewadmin', 'block_vxg_orgs', ''))),
                array('style' => 'pointer-events:none;', 'id' => 'vxg_org_viewadmin_btn'));

            $buttons .= $addadminbtn . $viewadminbtn;
        }
