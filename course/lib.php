...
function course_update_section($course, $section, $data) {
    ...
    if (array_key_exists('name', $data) && \core_text::strlen($data['name']) > 1000) {
        throw new moodle_exception('maximumchars', 'moodle', '', 1000);
    }
...
function course_get_cm_edit_actions(cm_info $mod, $indent = -1, $sr = null) {
    ...
    // Delete.
    /*$roles = get_user_roles($modcontext,$USER->id);
    foreach ($roles as $role){
        $roleids = $role->roleid;
    }*/
    if ($hasmanageactivities/* && $roleids != 10*/) {
