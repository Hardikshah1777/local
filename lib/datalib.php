...
function get_users($get=true, $search='', $confirmed=false, array $exceptions=null, $sort='firstname ASC',
                   $firstinitial='', $lastinitial='', $page='', $recordsperpage='', $fields='*', $extraselect='', array $extraparams=null /*, $userauthstatus = null*/) {
    global $DB, $CFG;
    ...
    if ($extraselect) {
        ...
        if (!empty($extraselect)) {
            if ($extraselect == 1) {
                $select .= ' AND id IN (SELECT userid FROM {tool_mfa_skipusers} WHERE status = 1)';
            } elseif ($extraselect == 2) {
                $select .= ' AND id NOT IN (SELECT userid FROM {tool_mfa_skipusers} WHERE status = 1)';
            }
        }
        $params = $params + (array)$extraparams;
    }

//    if (!empty($extraparams['userauthstatus'])) {
//        if ($extraparams['userauthstatus'] == 1) {
//            $select .= ' AND id IN (SELECT userid FROM {tool_mfa_skipusers} WHERE status = 1)';
//        } elseif ($extraparams['userauthstatus'] == 2) {
//            $select .= ' AND id NOT IN (SELECT userid FROM {tool_mfa_skipusers} WHERE status = 1)';
//        }
//    }
...
function get_users_listing($sort='lastaccess', $dir='ASC', $page=0, $recordsperpage=0,
                           ...
                           array $extraparams=null, $extracontext = null, $userauthstatus = null) {
    global $DB, $CFG;
    ...
    if (!empty($userauthstatus)) {
        if ($userauthstatus == 1) {
            $join .= ' JOIN {tool_mfa_skipusers} tm ON tm.userid = u.id AND tm.status = 1';
        } elseif ($userauthstatus == 2) {
            $select .= '  AND u.id NOT IN (SELECT userid FROM {tool_mfa_skipusers} WHERE status = 1)';
        }
    }
