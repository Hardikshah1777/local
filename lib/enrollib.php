...
abstract class enrol_plugin {
    ...
    public function enrol_user(stdClass $instance, $userid, $roleid = null, $timestart = 0, $timeend = 0, $status = null, $recovergrades = null) {
        ...
        $maxstudent = $DB->get_record('course', ['id' => $courseid], 'maxstudent');
        $totalusers = COUNT(get_enrolled_users($context));
        ...
        if ($ue = $DB->get_record('user_enrolments', array('enrolid'=>$instance->id, 'userid'=>$userid))) {
            ...
            if (!empty($maxstudent->maxstudent)) {
                if ($maxstudent->maxstudent > $totalusers) {
                    $ue->id = $DB->insert_record('user_enrolments', $ue);
                } else {
                    throw new enrol_maxlimit_exception();
                }
            } else {
                $ue->id = $DB->insert_record('user_enrolments', $ue);
            }
