...
//            (cc.timecompleted + ".$DB->sql_cast_char2int('r2.value').") < ?";
        $sql = " SELECT qd.uniqueid,qd.userid, c.id as course,qd.timefinish
            FROM {course} c
                JOIN (
                SELECT q.course,qa.userid,MAX(qa.timefinish) AS timefinish,MAX(qa.id) AS uniqueid
                FROM {quiz} q JOIN {quiz_attempts} qa ON qa.quiz = q.id
                WHERE qa.timefinish > 0 AND qa.state = 'finished'
                GROUP BY q.course, qa.userid
                ) qd ON qd.course = c.id
            JOIN {local_recompletion_config} r ON r.course = c.id AND r.name = 'enable' AND r.value = '1'
            JOIN {local_recompletion_config} r2 ON r2.course = c.id AND r2.name = 'recompletionduration'
            WHERE c.enablecompletion = 1
            AND (qd.timefinish + " . $DB->sql_cast_char2int( 'r2.value' ) . ") < ?";
    ...
    protected function notify_user($userid, $course, $config) {
        ...
        mtrace('uncomment code for recompletion');
