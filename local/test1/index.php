...
class userlist extends table_sql {
    ...
    public function col_timecreated($row) {
        return userdate( $row->timecreated, get_string( 'strftimedatetimeshortaccurate', 'core_langconfig' ));
    }
    ...
    public function col_action($row) {
        ...
        $mail = new moodle_url('/local/test1/testmail.php', ['uid' => $row->id, 'search'=> $this->search]);
        $email = $OUTPUT->action_link($mail,new pix_icon('t/email',get_string( 'email')));
        return ($edituser . $email .$deleteuser);
    }
...
$userlisttable->set_sql('id, username, firstname, lastname, email, city, timecreated',
                '{user}',
               'id > 2 AND deleted = 0 AND suspended = 0'.$where,
                      $params);
