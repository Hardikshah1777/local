YUI.add('moodle-assignfeedback_editpdf-editor1', function (Y, NAME) {
...
Y.extend(ANNOTATION, Y.Base, {
    ...
    remove: function(e) {
        ...
        annotations = this.editor.pages[this.editor.viewpage].annotations;
...
var COMMENT = function(editor, gradeid, pageno, x, y, width, colour, rawtext) {
    ...
    this.remove = function() {
        ...
        comments = this.editor.pages[this.editor.viewpage].comments;
 ...
 * @extends Y.Base
 */
AJAXBASE = M.cfg.wwwroot + '/mod/assign/feedback/editpdf/ajax1.php';
var EDITOR = function() {
...
EDITOR.prototype = {
    ...
    loadPages: function() {
        var background, pageX, pageY, pageIndex, drawingcanvas, _page;
        background = '';
        pageX = 0;
        pageY = 0;
        for (pageIndex = 0; pageIndex < this.pagecount; pageIndex++) {
            _page = this.pages[pageIndex];
            _page.pageY = pageY;
            background += 'url("' + _page.url + '") no-repeat center top ' + pageY + 'px,';
            pageY += (_page.height + this.pageSpacing);
            pageX = Math.max(pageX, _page.width);
        }
        background = background.replace(/,$/, '');
        drawingcanvas = this.get_dialogue_element(SELECTOR.DRAWINGCANVAS);
        drawingcanvas.setStyle('background', background);
        drawingcanvas.setStyle('width', pageX + 'px');
        drawingcanvas.setStyle('height', pageY + 'px');
    },
    ...
    stringify_current_page: function() {
        ...
        for (i = 0; i < this.pages[this.viewpage].comments.length; i++) {
            comments[i] = this.pages[this.viewpage].comments[i].clean();
        }
        for (i = 0; i < this.pages[this.viewpage].annotations.length; i++) {
            annotations[i] = this.pages[this.viewpage].annotations[i].clean();
        }
    ...
    edit_start: function(e) {
        ...
        if (this.currentedit.tool === 'select') {
            var x = this.currentedit.end.x,
                ...
                annotations = this.pages[this.viewpage].annotations;
    ...
    edit_end: function() {
        ...
        if (this.currentedit.tool === 'comment') {
            ...
            if (comment.init_from_edit(this.currentedit)) {
                this.pages[this.viewpage].comments.push(comment);
            ...
            if (annotation) {
                ...
                if (annotation.init_from_edit(this.currentedit)) {
                    this.pages[this.viewpage].annotations.push(annotation);
    ...
    change_page: function() {
        ...
        drawingregion.set('scrollTop', page.pageY);
    ...
    setup_navigation: function() {
        ...
        var drawingregion = this.get_dialogue_element(SELECTOR.DRAWINGREGION);
        drawingregion.on('scroll', function(e) {
            var scrollTop = e.target.get('scrollTop');
            var activepage = 0;
            for (var pageIndex in this.pages) {
                if (this.pages[pageIndex].pageY > scrollTop) {
                    break;
                }
                activepage = pageIndex;
            }
            this.currentpage = activepage;
            pageselect.set('selectedIndex', activepage);
        }, this);
    },
    ...
    rotatePDF: function(e, left) {
        ...
        var annotations = this.pages[this.viewpage].annotations;
        for (i = 0; i < annotations.length; i++) {
        ...
        var config = {
            ...
            on: {
                success: function(tid, response) {
                    var jsondata;
                    try {
                        var page = self.pages[self.viewpage];
                        jsondata = Y.JSON.parse(response.responseText);
                        if (jsondata.pages) {
                            self.pagecount = jsondata.pages.length;
                            for (var pageIndex = 0; pageIndex < self.pagecount; pageIndex++) {
                                self.pages[pageIndex] = Object.assign(self.pages[pageIndex], jsondata.pages[pageIndex]);
                            }
                        }
...
M.assignfeedback_editpdf = M.assignfeedback_editpdf || {};
M.assignfeedback_editpdf.editor1 = M.assignfeedback_editpdf.editor1 || {};
 ...
 * @param {Object} params
 */
M.assignfeedback_editpdf.editor1.init = M.assignfeedback_editpdf.editor1.init || function(params) {
