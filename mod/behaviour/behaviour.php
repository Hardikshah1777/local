...
$attforsession = $DB->get_record('behaviour_sessions', array('id' => $id), '*', MUST_EXIST);
$attconfig = get_config('behaviour');
$behaviour = $DB->get_record('behaviour', array('id' => $attforsession->behaviourid), '*', MUST_EXIST);
...
if (!empty($attforsession->groupid) && !groups_is_member($attforsession->groupid, $USER->id)) {
    throw new moodle_exception('cannottakethisgroup', 'behaviour');
}

if ($DB->record_exists('behaviour_log', ['sessionid' => $id, 'studentid' => $USER->id])) {
    $url = new moodle_url('/mod/behaviour/view.php', ['id' => $cm->id]);
    throw new moodle_exception('behaviour_already_submitted', 'mod_behaviour', $url);
}
...
if ($attforsession->rotateqrcode == 1) {
    $cookiename = 'behaviour_'.$attforsession->id;
    ...
    if (isset($_COOKIE[$cookiename])) {
        ...
        if ($secrethash !== $_COOKIE[$cookiename]) {
            // Flag error.
            throw new moodle_exception('qr_cookie_error', 'mod_behaviour', $url);
        }
    } else {
        // Check password.
        $sql = 'SELECT * FROM {behaviour_rotate_passwords}'.
            ' WHERE behaviourid = ? AND expirytime > ? ORDER BY expirytime ASC';
        $qrpassdatabase = $DB->get_records_sql($sql, ['behaviourid' => $id, time() - $attconfig->rotateqrcodeexpirymargin], 0, 2);
        ...
        if ($qrpassflag) {
            ...
            if (debugging('', DEBUG_DEVELOPER)) {
                // Get qr code record.
                $rec = $DB->get_record('behaviour_rotate_passwords', ['behaviourid' => $id, 'password' => $qrpass]);
                if (empty($rec)) {
            ...
            throw new moodle_exception('qr_pass_wrong', 'mod_behaviour', $url);
        }
    }
}

list($canmark, $reason) = behaviour_can_student_mark($attforsession);
if (!$canmark) {
    redirect(new moodle_url('/mod/behaviour/view.php', array('id' => $cm->id)), get_string($reason, 'behaviour'));
    exit;
}
...
if (!empty($attforsession->subnet) && !address_in_subnet(getremoteaddr(), $attforsession->subnet)) {
    $url = new moodle_url('/mod/behaviour/view.php', array('id' => $cm->id));
    notice(get_string('subnetwrong', 'behaviour'), $url);
    exit; // Notice calls this anyway.
}

$pageparams->sessionid = $id;
$att = new mod_behaviour_structure($behaviour, $cm, $course, $PAGE->context, $pageparams);
...
if ($attforsession->autoassignstatus && (empty($attforsession->studentpassword)) || $qrpassflag) {
    $statusid = behaviour_session_get_highest_status($att, $attforsession);
    $url = new moodle_url('/mod/behaviour/view.php', array('id' => $cm->id));
    if (empty($statusid)) {
        throw new moodle_exception('behaviour_no_status', 'mod_behaviour', $url);
    }
    $take = new stdClass();
    ...
    if ($success) {
        ...
        redirect($url, get_string('studentmarked', 'behaviour'));
    } else {
        throw new moodle_exception('behaviour_already_submitted', 'mod_behaviour', $url);
    }
}
...
if (!empty($qrpass) && !empty($attforsession->autoassignstatus)) {
    ...
    if (!empty($attforsession->studentpassword) &&
        ...
        $url = new moodle_url('/mod/behaviour/behaviour.php', array('sessid' => $id, 'sesskey' => sesskey()));
        redirect($url, get_string('incorrectpassword', 'mod_behaviour'), null, \core\output\notification::NOTIFY_ERROR);
    }

    // Set the password and session id in the form, because they are saved in the behaviour log.
    $fromform->studentpassword = $qrpass;
    $fromform->sessid = $attforsession->id;

    $fromform->status = behaviour_session_get_highest_status($att, $attforsession);
...
// Create the form.
if ($attforsession->rotateqrcode == 1) {
    $mform = new mod_behaviour\form\studentbehaviour(null,
        array('course' => $course, 'cm' => $cm, 'modcontext' => $PAGE->context, 'session' => $attforsession,
            'behaviour' => $att, 'password' => $attforsession->studentpassword));
} else {
    $mform = new mod_behaviour\form\studentbehaviour(null,
        array('course' => $course, 'cm' => $cm, 'modcontext' => $PAGE->context, 'session' => $attforsession,
            'behaviour' => $att, 'password' => $qrpass));
}
