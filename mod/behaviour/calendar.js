        ...
        Event.on(showBtn, "click", function() {
            ...
            if (!dialog) {
                ...
                dialog = new YAHOO.widget.Dialog("attcalendarcontainer", {
                    visible: false,
                    context: ["show", "tl", "bl"],
                    buttons: [{text: M.util.get_string('calclose', 'behaviour'), handler: closeHandler}],
                    draggable: false,
                    close: false
                });
                dialog.setHeader('');
            ...
            if (!calendar) {
                ...
                calendar.cfg.setProperty("start_weekday", M.behaviour.cal_start_weekday);
                calendar.cfg.setProperty("MONTHS_LONG", M.behaviour.cal_months);
                calendar.cfg.setProperty("WEEKDAYS_SHORT", M.behaviour.cal_week_days);
                calendar.select(new Date(M.behaviour.cal_cur_date * 1000));
                calendar.render();
