 ...
 */
function behaviour_create_calendar_event(&$session) {
    global $DB;
    ...
    if (empty(get_config('behaviour', 'enablecalendar')) || $session->calendarevent === 0) {
        // Calendar events are not used, or event not required for this session.
        return true;
    }

    $behaviour = $DB->get_record('behaviour', array('id' => $session->behaviourid));

    $caleventdata = new stdClass();
    $caleventdata->name           = $behaviour->name;
    $caleventdata->courseid       = $behaviour->course;
    $caleventdata->groupid        = $session->groupid;
    $caleventdata->instance       = $session->behaviourid;
    ...
    $caleventdata->eventtype      = 'behaviour';
    $caleventdata->timemodified   = time();
    $caleventdata->modulename     = 'behaviour';
    ...
    if ($calevent = calendar_event::create($caleventdata, false)) {
        ...
        $DB->set_field('behaviour_sessions', 'caleventid', $session->caleventid, array('id' => $session->id));
        return true;
    } else {
        return false;
    }
 ...
 */
function behaviour_create_calendar_events($sessionsids) {
    global $DB;
 ...
 */
function behaviour_update_calendar_event($session) {
    global $DB;
    ...
    if ($session->caleventid == 0) {
        return behaviour_create_calendar_event($session);
    }
 ...
 * @return bool result of updating
 */
function behaviour_delete_calendar_events($sessionsids) {
    global $DB;
    $caleventsids = behaviour_existing_calendar_events_ids($sessionsids);
    if ($caleventsids) {
 ...
 */
function behaviour_existing_calendar_events_ids($sessionsids) {
    global $DB;
    $caleventsids = array_keys($DB->get_records_list('behaviour_sessions', 'id', $sessionsids, '', 'caleventid'));
