...
namespace mod_behaviour\customfield;
...
class session_handler extends \core_customfield\handler {
    ...
    public function get_configuration_url() : \moodle_url {
        return new \moodle_url('/mod/behaviour/customfield.php');
    }
    ...
    public function get_instance_context(int $instanceid = 0) : \context {
        global $DB;
        if ($instanceid > 0) {
            $behaviourid = $DB->get_field('behaviour_sessions', 'behaviourid', ['id' => $instanceid]);
    ...
    /**
     * Aehaviour session form a bit non-standard, use custom function to populate array of existing data.
     *
     * Example:
    ...
    /**
     * Get list of custom fields that contain data in this behaviour activity (hides fields that do not store anything)
     *
     * @param int $instanceid id of the record to get the context for
     * @return array
     */
    ...
    public function get_instance_data_for_backup_by_activity(int $activityid) : array {
        global $DB;
        $finalfields = [];
        $sessions = $DB->get_records('behaviour_sessions', ['behaviourid' => $activityid]);
        if (empty($sessions)) {
            return $finalfields;
        }
