...
class addsession extends moodleform {
    ...
    public function definition() {
        ...
        $mform->addElement('header', 'general', get_string('addsession', 'behaviour'));

        $groupmode = groups_get_activity_groupmode($cm);
        switch ($groupmode) {
            case NOGROUPS:
                $mform->addElement('static', 'sessiontypedescription', get_string('sessiontype', 'behaviour'),
                                  get_string('commonsession', 'behaviour'));
                $mform->addHelpButton('sessiontypedescription', 'sessiontype', 'behaviour');
                $mform->addElement('hidden', 'sessiontype', mod_behaviour_structure::SESSION_COMMON);
                $mform->setType('sessiontype', PARAM_INT);
                break;
            case SEPARATEGROUPS:
                $mform->addElement('static', 'sessiontypedescription', get_string('sessiontype', 'behaviour'),
                                  get_string('groupsession', 'behaviour'));
                $mform->addHelpButton('sessiontypedescription', 'sessiontype', 'behaviour');
                $mform->addElement('hidden', 'sessiontype', mod_behaviour_structure::SESSION_GROUP);
                $mform->setType('sessiontype', PARAM_INT);
                break;
            case VISIBLEGROUPS:
                $radio = array();
                $radio[] = &$mform->createElement('radio', 'sessiontype', '', get_string('commonsession', 'behaviour'),
                                                  mod_behaviour_structure::SESSION_COMMON);
                $radio[] = &$mform->createElement('radio', 'sessiontype', '', get_string('groupsession', 'behaviour'),
                                                  mod_behaviour_structure::SESSION_GROUP);
                $mform->addGroup($radio, 'sessiontype', get_string('sessiontype', 'behaviour'), ' ', false);
                $mform->setType('sessiontype', PARAM_INT);
                $mform->addHelpButton('sessiontype', 'sessiontype', 'behaviour');
                $mform->setDefault('sessiontype', mod_behaviour_structure::SESSION_COMMON);
                break;
        }
        ...
        if ($maxstatusset > 0) {
            ...
            for ($i = 0; $i <= $maxstatusset; $i++) {
                $opts[$i] = behaviour_get_setname($this->_customdata['att']->id, $i);
            }
            $mform->addElement('select', 'statusset', get_string('usestatusset', 'mod_behaviour'), $opts);
        } else {
            $mform->addElement('hidden', 'statusset', 0);
            $mform->setType('statusset', PARAM_INT);
        }

        $mform->addElement('editor', 'sdescription', get_string('description', 'behaviour'), array('rows' => 1, 'columns' => 80),
        ...
        $mform->addElement('header', 'headeraddmultiplesessions', get_string('addmultiplesessions', 'behaviour'));
        if (!empty($pluginconfig->multisessionexpanded)) {
            $mform->setExpanded('headeraddmultiplesessions');
        }
        $mform->addElement('checkbox', 'addmultiply', '', get_string('repeatasfollows', 'behaviour'));
        $mform->addHelpButton('addmultiply', 'createmultiplesessions', 'behaviour');
        ...
        $mform->addGroup($sdays, 'sdays', get_string('repeaton', 'behaviour'), array('&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;'), true);
        ...
        $periodgroup[] =& $mform->createElement('static', 'perioddesc', '', get_string('week', 'behaviour'));
        $mform->addGroup($periodgroup, 'periodgroup', get_string('repeatevery', 'behaviour'), array(' '), false);
        $mform->disabledIf('periodgroup', 'addmultiply', 'notchecked');

        $mform->addElement('date_selector', 'sessionenddate', get_string('repeatuntil', 'behaviour'));
        ...
        if ($DB->record_exists('behaviour_statuses', ['behaviourid' => $this->_customdata['att']->id, 'setunmarked' => 1])) {
            ...
            $mform->addElement('select', 'automark', get_string('automark', 'behaviour'), $options);
            ...
            $automarkcmoptions = behaviour_get_coursemodulenames($course->id);

            $mform->addElement('select', 'automarkcmid', get_string('selectactivity', 'behaviour'), $automarkcmoptions);
        ...
        if (!empty($studentscanmark)) {
            ...
            $mgroup[] = & $mform->createElement('text', 'studentpassword', get_string('studentpassword', 'behaviour'));
            $mform->disabledif('studentpassword', 'rotateqrcode', 'checked');
            $mgroup[] = & $mform->createElement('checkbox', 'randompassword', '', get_string('randompassword', 'behaviour'));
            $mform->disabledif('randompassword', 'rotateqrcode', 'checked');
            $mgroup[] = & $mform->createElement('checkbox', 'includeqrcode', '', get_string('includeqrcode', 'behaviour'));
            $mform->disabledif('includeqrcode', 'rotateqrcode', 'checked');

            $mform->addGroup($mgroup, 'passwordgrp', get_string('passwordgrp', 'behaviour'), array(' '), false);

            $mform->setType('studentpassword', PARAM_TEXT);
            $mform->addHelpButton('passwordgrp', 'passwordgrp', 'behaviour');
            ...
            $mform->hideif('passwordgrp', 'automark', 'eq', BEHAVIOUR_AUTOMARK_ALL);
        ...
        $mgroup2 = array();
        $mgroup2[] = & $mform->createElement('text', 'subnet', get_string('requiresubnet', 'behaviour'));
        if (empty(get_config('behaviour', 'subnetactivitylevel'))) {
            $mform->setDefault('subnet', get_config('behaviour', 'subnet'));
        } else {
            $mform->setDefault('subnet', $this->_customdata['att']->subnet);
        }

        $mgroup2[] = & $mform->createElement('checkbox', 'usedefaultsubnet', get_string('usedefaultsubnet', 'behaviour'));
        $mform->setDefault('usedefaultsubnet', 1);
        $mform->setType('subnet', PARAM_TEXT);

        $mform->addGroup($mgroup2, 'subnetgrp', get_string('requiresubnet', 'behaviour'), array(' '), false);
        ...
        $mform->addGroup($mgroup3, 'preventsharedgroup', get_string('preventsharedip', 'behaviour'), array(' '), false);
        ...
        $this->add_action_buttons(true, get_string('add', 'behaviour'));
    }
    ...
    public function validation($data, $files) {
        ...
        if (!empty($data['addmultiply']) && $data['sessiondate'] != 0 && $data['sessionenddate'] != 0 &&
            ...
            $errors['sessionenddate'] = get_string('invalidsessionenddate', 'behaviour');
        }

        if ($data['sessiontype'] == mod_behaviour_structure::SESSION_GROUP && empty($data['groups'])) {
            $errors['groups'] = get_string('errorgroupsnotselected', 'behaviour');
        }

        $addmulti = isset($data['addmultiply']) ? (int)$data['addmultiply'] : 0;
        if (($addmulti != 0) && (!array_key_exists('sdays', $data) || empty($data['sdays']))) {
            $data['sdays'] = array();
            $errors['sdays'] = get_string('required', 'behaviour');
        }
        if (isset($data['sdays'])) {
            if (!$this->checkweekdays($data['sessiondate'], $data['sessionenddate'], $data['sdays']) ) {
                $errors['sdays'] = get_string('checkweekdays', 'behaviour');
            }
        }
        if ($addmulti && ceil(($data['sessionenddate'] - $data['sessiondate']) / YEARSECS) > 1) {
            $errors['sessionenddate'] = get_string('timeahead', 'behaviour');
        }
        $sessstart = $data['sessiondate'] + $sesstarttime;
        if ($sessstart < $data['coursestartdate'] && $sessstart != $data['previoussessiondate']) {
            $errors['sessiondate'] = get_string('priorto', 'behaviour',
                userdate($data['coursestartdate'], get_string('strftimedmyhm', 'behaviour')));
