...
class duration extends moodleform {
    ...
    public function definition() {
        ...
        $mform->addElement('header', 'general', get_string('changeduration', 'behaviour'));
        $mform->addElement('static', 'count', get_string('countofselected', 'behaviour'), count(explode('_', $ids)));
        ...
        $mform->addGroup($durselect, 'durtime', get_string('newduration', 'behaviour'), array(' '), true);

        $mform->addElement('hidden', 'ids', $ids);
        $mform->setType('ids', PARAM_ALPHANUMEXT);
        $mform->addElement('hidden', 'id', $cm->id);
        $mform->setType('id', PARAM_INT);
        $mform->addElement('hidden', 'action', \mod_behaviour_sessions_page_params::ACTION_CHANGE_DURATION);
        $mform->setType('action', PARAM_INT);

        $mform->setDefaults(array('durtime' => array('hours' => 0, 'minutes' => 0)));

        $submitstring = get_string('update', 'behaviour');
