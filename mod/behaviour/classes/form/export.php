...
/**
 * Export behaviour sessions forms
 *
...
class export extends \moodleform {
    ...
    public function definition() {
        ...
        $mform->addElement('header', 'general', get_string('export', 'behaviour'));
        ...
        $allusers = get_enrolled_users($modcontext, 'mod/behaviour:canbelisted', 0, 'u.id,'.$namefields);
        $userlist = array();
        foreach ($allusers as $user) {
            $userlist[$user->id] = fullname($user);
        }
        unset($allusers);
        $tempusers = $DB->get_records('behaviour_tempusers', array('courseid' => $course->id), 'studentid, fullname');
        foreach ($tempusers as $user) {
            $userlist[$user->studentid] = $user->fullname;
        }
        if (empty($userlist)) {
            $mform->addElement('static', 'nousers', '', get_string('nobehaviourusers', 'behaviour'));
            return;
        }
        ...
        $mform->addElement('selectyesno', 'selectedusers', get_string('onlyselectedusers', 'mod_behaviour'));
        $sel = $mform->addElement('select', 'users', get_string('users', 'mod_behaviour'), $userlist, array('size' => 12));
        $sel->setMultiple(true);
        $mform->disabledIf('users', 'selectedusers', 'eq', 0);

        $opts = array('groupmappings' => $groupmappings);
        $PAGE->requires->yui_module('moodle-mod_behaviour-groupfilter', 'M.mod_behaviour.groupfilter.init', array($opts));

        $ident = array();
        $checkedfields = array();

        $adminsetfields = get_config('behaviour', 'customexportfields');
        if (in_array('id', explode(',', $adminsetfields))) {
            $ident[] =& $mform->createElement('checkbox', 'id', '', get_string('studentid', 'behaviour'));
        ...
        if (count($ident) > 0) {
            $mform->addGroup($ident, 'ident', get_string('identifyby', 'behaviour'), array('<br />'), true);
            $mform->setDefaults($checkedfields);
        }
        $mform->setType('id', PARAM_INT);

        $mform->addElement('checkbox', 'includeallsessions', get_string('includeall', 'behaviour'), get_string('yes'));
        $mform->setDefault('includeallsessions', true);
        $mform->addElement('checkbox', 'includenottaken', get_string('includenottaken', 'behaviour'), get_string('yes'));
        $mform->addElement('checkbox', 'includeremarks', get_string('includeremarks', 'behaviour'), get_string('yes'));
        $mform->addElement('checkbox', 'includedescription', get_string('includedescription', 'behaviour'), get_string('yes'));
        $mform->addElement('date_selector', 'sessionstartdate', get_string('startofperiod', 'behaviour'));
        $mform->setDefault('sessionstartdate', $course->startdate);
        $mform->disabledIf('sessionstartdate', 'includeallsessions', 'checked');
        $mform->addElement('date_selector', 'sessionenddate', get_string('endofperiod', 'behaviour'));
        $mform->disabledIf('sessionenddate', 'includeallsessions', 'checked');

        $formatoptions = array('excel' => get_string('downloadexcel', 'behaviour'),
                               'ooo' => get_string('downloadooo', 'behaviour'),
                               'text' => get_string('downloadtext', 'behaviour'));
    ...
    public function validation($data, $files) {
        ...
        if ($data['selectedusers'] && empty($data['users'])) {
            $errors['users'] = get_string('mustselectusers', 'mod_behaviour');
        }
