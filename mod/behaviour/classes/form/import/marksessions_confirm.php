...
/**
 * Mark behaviour sessions confirm csv upload.
 *
 * @package   mod_behaviour
 * @copyright 2020 Catalyst IT
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
...
class marksessions_confirm extends moodleform {
    ...
    public function definition() {
        ...
        // Add user mapping.
        $mform->addElement('select', 'userfrom', get_string('userimportfield', 'behaviour'), $foundheaders);
        $mform->addHelpButton('userfrom', 'userimportfield', 'behaviour');
        // This allows the user to choose which field in the user database the identifying column will map to.
        $useroptions = array(
            'id'       => get_string('userid', 'behaviour'),
            'username'     => get_string('username'),
            'idnumber' => get_string('idnumber'),
            'email'    => get_string('email')
        );
        $mform->addElement('select', 'userto', get_string('userimportto', 'behaviour'), $useroptions);
        ...
        $mform->addHelpButton('userto', 'userimportto', 'behaviour');

        // Below options need a "none" option in the headers.
        $foundheaders[- 1] = get_string('notset', 'mod_behaviour');
        ksort($foundheaders);

        // Add scan time mapping.
        $mform->addElement('select', 'scantime', get_string('scantime', 'behaviour'), $foundheaders);
        $mform->addHelpButton('scantime', 'scantime', 'behaviour');
        $mform->setDefault('scantime', -1);

        // Add status mapping.
        $mform->addElement('select', 'status', get_string('importstatus', 'behaviour'), $foundheaders);
        $mform->addHelpButton('status', 'importstatus', 'behaviour');
