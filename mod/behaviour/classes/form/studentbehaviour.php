 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class studentbehaviour extends \moodleform {
    /**
    ...
    public function definition() {
        ...
        $attblock = $this->_customdata['behaviour'];
        ...
        $sesstiontitle = userdate($attforsession->sessdate, get_string('strftimedate')).' '
                .behaviour_strftimehm($attforsession->sessdate);
        ...
        if (!empty($attforsession->studentpassword)) {
            $mform->addElement('text', 'studentpassword', get_string('password', 'behaviour'));
            $mform->setType('studentpassword', PARAM_TEXT);
            $mform->addRule('studentpassword', get_string('passwordrequired', 'behaviour'), 'required');
            $mform->setDefault('studentpassword', $password);
        }
        if (!$attforsession->autoassignstatus) {

            // Create radio buttons for setting the behaviour status.
            $radioarray = array();
            foreach ($statuses as $status) {
            ...
            if ($disabledduetotime) {
                $warning = \html_writer::span(get_string('somedisabledstatus', 'behaviour'), 'somedisabledstatus');
            ...
            $mform->addRule('statusarray', get_string('behaviournotset', 'behaviour'), 'required', '', 'client', false, false);
        }
    ...
    public function validation($data, $files) {
        ...
        if (!($this->_customdata['session']->autoassignstatus)) {
            ...
            if (empty($data['status'])) {
                $errors['statusarray'] = get_string('invalidstatus', 'behaviour');
            }
        }
