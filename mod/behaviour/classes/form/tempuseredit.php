...
class tempuseredit extends \moodleform {
    ...
    public function definition() {
        ...
        $mform->addElement('header', 'attheader', get_string('tempusersedit', 'behaviour'));
        $mform->addElement('text', 'tname', get_string('tusername', 'behaviour'));
        $mform->addRule('tname', 'Required', 'required', null, 'client');
        $mform->setType('tname', PARAM_TEXT);

        $mform->addElement('text', 'temail', get_string('tuseremail', 'behaviour'));
        $mform->addRule('temail', 'Email', 'email', null, 'client');
        $mform->setType('temail', PARAM_EMAIL);

        $buttonarray = array(
            $mform->createElement('submit', 'submitbutton', get_string('edituser', 'behaviour')),
    ...
    public function validation($data, $files) {
        ...
        if ($err = \mod_behaviour_structure::check_existing_email($data['temail'], $data['userid'])) {
            $errors['temail'] = $err;
        }
        return $errors;
    }
