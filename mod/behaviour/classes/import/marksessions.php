...
class marksessions {
    ...
    /** @var mod_behaviour_structure $att - the mod_behaviour_structure class */
    private $att;
     ...
     * @param mod_behaviour_structure $att The current assignment
    ...
    public function __construct($text = null, $att, $encoding = null, $delimiter = null, $importid = 0,
        ...
        while ($row = $this->importer->next()) {
            ...
            if (empty($users[$extuser])) {
                ...
                \mod_behaviour_notifyqueue::notify_problem(get_string('error:usernotfound', 'behaviour', $a));
                continue;
            }
            $userid = $users[$extuser]->id;
            if (isset($sesslog[$userid])) {
                \mod_behaviour_notifyqueue::notify_problem(get_string('error:userduplicate', 'behaviour', $extuser));
                continue;
            }
            ...
            if (!empty($scantime)) {
                ...
                if ($t === false) {
                    ...
                    $a->extuser = $extuser;
                    $a->scantime = $scantime;
                    \mod_behaviour_notifyqueue::notify_problem(get_string('error:timenotreadable', 'behaviour', $a));
                    continue;
                }

                $sesslog[$userid]->statusid = behaviour_session_get_highest_status($this->att, $sessioninfo, $t);
            } else {
                ...
                if (!empty($statusmap[$status])) {
                    ...
                    $a->extuser = $extuser;
                    $a->status = $status;
                    \mod_behaviour_notifyqueue::notify_problem(get_string('error:statusnotfound', 'behaviour', $a));
                    continue;
                }
            }
        }
    ...
    public function import() {
        ...
        \mod_behaviour_notifyqueue::notify_success(get_string('sessionsupdated', 'mod_behaviour'));
    }
}
