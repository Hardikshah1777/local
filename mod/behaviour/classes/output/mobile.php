...
/**
 * Contains the mobile output class for the behaviour
 *
...
/**
 * Mobile output class for the behaviour.
 *
 * @copyright 2018 Dan Marsden
 * @license http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class mobile {
    ...
    public static function mobile_view_activity($args) {
        ...
        // Capabilities check.
        $cm = get_coursemodule_from_id('behaviour', $cmid);
        ...
        $data['timestamp'] = time(); // Used to prevent behaviour session marking page to be cached.

        $data['behaviourfunction'] = 'mobile_user_form';
        $isteacher = false;
        if (has_capability('mod/behaviour:takebehaviours', $context)) {
            $isteacher = true;
            $data['behaviourfunction'] = 'mobile_teacher_form';
        }
        ...
        if (!empty($sessid) && (!empty($takenstatus) || $isteacher)) {
            $session = $DB->get_record('behaviour_sessions', array('id' => $sessid));
        ...
        $pageparams->mode = \mod_behaviour_view_page_params::MODE_THIS_COURSE;
        ...
        if ($isteacher && $canseegroupsession) {
            ...
            if (!empty($userkeys)) { // If this is a post from the teacher form.
                ...
                $data['messages'][]['string'] = 'behavioursuccess';
            }
        }
        ...
        $sessions = $DB->get_records_select('behaviour_sessions',
            'behaviourid = ? AND sessdate > ? AND sessdate < ? ORDER BY sessdate',
            array($behaviour->id, $timefrom, $timeto));
        ...
        if (!empty($sessions)) {
            ...
            foreach ($sessions as $sess) {
                ...
                list($canmark, $reason) = behaviour_can_student_mark($sess);
                ...
                if ($isteacher || $canmark) {
                    $html = array('time' => strip_tags(mod_behaviour_construct_session_full_date_time($sess->sessdate, $sess->duration)),
                        'groupname' => '');
                    ...
                    if (!$isteacher && !empty($userdata->sessionslog['c'.$sess->id]->statusid)) {
                        ...
                        if (!$isteacher) {
                            if (!empty($sess->subnet) && !address_in_subnet(getremoteaddr(), $sess->subnet)) {
                                ...
                                $statusid = behaviour_session_get_highest_status($att, $sess);
                                if (empty($statusid)) {
                                    $data['showmessage'] = true;
                                    $data['messages'][]['string'] = 'behaviour_no_status';
                                }
                                $take = new \stdClass();
                                ...
                                if (!empty($sess->studentpassword) && $password != $sess->studentpassword) {
                                    ...
                                    if ($sess->autoassignstatus) {
                                        ...
                                        $takenstatus = behaviour_session_get_highest_status($att, $sess);
                                    }
        ...
        $summary = new \mod_behaviour_summary($att->id, array($USER->id), $att->pageparams->startdate,
        ...
        return [
            'templates' => [
                [
                    'id' => 'main',
                    'html' => $OUTPUT->render_from_template("mod_behaviour/mobile_view_page_$versionname", $data),
                ],
            ],
            'javascript' => '',
            'otherdata' => ''
    ...
    /**
     * Returns the form to take behaviour for the mobile app.
     *
     * @param  array $args Arguments from tool_mobile_get_content WS
     * @return array HTML, javascript and other data
     */
    public static function mobile_user_form($args) {
        ...
        $behaviour    = $DB->get_record('behaviour', array('id' => $cm->instance), '*', MUST_EXIST);
        $attforsession = $DB->get_record('behaviour_sessions', array('id' => $sessid), '*', MUST_EXIST);
        $course        = $DB->get_record('course', array('id' => $cm->course), '*', MUST_EXIST);
        ...
        list($canmark, $reason) = behaviour_can_student_mark($attforsession, false);
        ...
        return [
            'templates' => [
                [
                    'id' => 'main',
                    'html' => $OUTPUT->render_from_template("mod_behaviour/mobile_user_form_$versionname", $data),
                    'cache-view' => false
                ],
            ],
            'javascript' => '',
    ...
    public static function mobile_teacher_form($args) {
        ...
        $statuses = $att->get_statuses();
        $otherdata = array();
        $existinglog = $DB->get_records('behaviour_log',
        ...
        return [
            'templates' => [
                [
                    'id' => 'main',
                    'html' => $OUTPUT->render_from_template("mod_behaviour/mobile_teacher_form_$versionname", $data),
                    'cache-view' => false
                ],
            ],
            'javascript' => '',
