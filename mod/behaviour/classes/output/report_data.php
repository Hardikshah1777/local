...
use mod_behaviour\local\url_helpers;
...
class report_data implements renderable {
    ...
    /** @var array|mod_behaviour_summary  */
    public $summary = array();
    /** @var mod_behaviour_structure  */
    public $att;
     ...
     */
    public function  __construct(mod_behaviour_structure $att) {
        ...
        $this->allstatuses = behaviour_get_statuses($att->id, false);

        if ($att->pageparams->view == ATT_VIEW_SUMMARY) {
            $this->summary = new mod_behaviour_summary($att->id);
        } else {
            $this->summary = new mod_behaviour_summary($att->id, array_keys($this->users),
        ...
        foreach ($this->users as $key => $user) {
            ...
            if ($att->pageparams->view != ATT_VIEW_NOTPRESENT ||
                behaviour_calc_fraction($usersummary->takensessionspoints, $usersummary->takensessionsmaxpoints) <
