...
/**
 * Behaviour module renderable component.
 *
...
class user_data implements renderable {
    ...
    /**
     * behaviour_user_data constructor.
     * @param mod_behaviour_structure $att
     * @param int $userid
     * @param boolean $mobile - this is called by the mobile code, don't generate everything.
     */
    public function  __construct(mod_behaviour_structure $att, $userid, $mobile = false) {
        $this->user = $att->get_user($userid);

        $this->pageparams = $att->pageparams;

        if ($this->pageparams->mode == mod_behaviour_view_page_params::MODE_THIS_COURSE) {
            $this->statuses = $att->get_statuses(true, true);

            if (!$mobile) {
                $this->summary = new mod_behaviour_summary($att->id, array($userid), $att->pageparams->startdate,
        ...
        } else if ($this->pageparams->mode == mod_behaviour_view_page_params::MODE_ALL_SESSIONS) {
            $this->coursesatts = behaviour_get_user_courses_behaviours($userid);
            $this->statuses = array();
            ...
            foreach ($this->coursesatts as $atid => $ca) {
                ...
                if (!$modinfo->instances['behaviour'][$ca->attid]->uservisible) {
                    unset($this->coursesatts[$atid]);
                    continue;
                } else {
                    $this->coursesatts[$atid]->cmid = $modinfo->instances['behaviour'][$ca->attid]->get_course_module_record()->id;
                }
                $this->statuses[$ca->attid] = behaviour_get_statuses($ca->attid);
                $this->summaries[$ca->attid] = new mod_behaviour_summary($ca->attid, array($userid));
            ...
            $this->sessionslog = behaviour_get_user_sessions_log_full($userid, $this->pageparams);

            foreach ($this->sessionslog as $sessid => $sess) {
                if (array_key_exists($sess->behaviourid, $this->coursesatts)) {
                    $this->sessionslog[$sessid]->cmid = $this->coursesatts[$sess->behaviourid]->cmid;
                } else {
                    // Session behaviourid not found in coursesatts, probably because it is not uservisible.
            ...
            foreach ($this->coursesatts as $atid => $ca) {
                ...
                $this->statuses[$ca->attid] = behaviour_get_statuses($ca->attid);
                $this->summary[$ca->attid] = new mod_behaviour_summary($ca->attid, array($userid));
            }
        }
    ...
    /**
     * Take multiple sessions behaviour from form data.
     *
    ...
    public function take_sessions_from_form_data($formdata) {
        ...
        foreach ($formdata as $key => $value) {
            ...
            if (substr($key, 0, 7) == 'remarks') {
                ...
                if (!has_capability('mod/behaviour:takebehaviours', $context)) {
                    ...
                    \core\notification::warning(get_string("nocapabilitytotakethisbehaviour", "behaviour", $dbsession->cmid));
                    continue;
                }

                $formkey = 'user'.$stid.'sess'.$sessid;
                $attid = $dbsession->behaviourid;
        ...
        foreach ($sesslog as $stid => $userlog) {
            $dbstudlog = $DB->get_records('behaviour_log', array('studentid' => $stid), '',
        ...
        if (!empty($updateatts)) {
            $behaviourgrade = $DB->get_records_list('behaviour', 'id', array_keys($updateatts), '', 'id, grade');
            foreach ($updateatts as $attid => $updateusers) {
                if ($behaviourgrade[$attid] != 0) {
                    behaviour_update_users_grades_by_id($attid, $grade, $updateusers);
                }
            }
        }
    }
