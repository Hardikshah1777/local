...
/**
 * Class definition for mod_behaviour_page_with_filter_controls
 *
 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class mod_behaviour_page_with_filter_controls {
    /** No filter. */
    ...
    public function init($cm) {
        ...
        if (empty($this->defaultview)) {
            $this->defaultview = get_config('behaviour', 'defaultview');
        }
        $this->init_view();
        $this->init_curdate();
    ...
    private function calc_sessgroupslist() {
        ...
        if ($allowedgroups) {
            if ($groupmode == VISIBLEGROUPS || has_capability('moodle/site:accessallgroups', $PAGE->context)) {
                $this->sessgroupslist[self::SESSTYPE_ALL] = get_string('all', 'behaviour');
            }
            // Show Common groups always.
            $this->sessgroupslist[self::SESSTYPE_COMMON] = get_string('commonsessions', 'behaviour');
