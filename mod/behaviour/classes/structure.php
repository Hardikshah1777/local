...
/**
 * Class definition for mod_behaviour_structure
 *
...
/**
 * Main class with all behaviour related info.
 *
 * @copyright  2016 Dan Marsden http://danmarsden.com
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class mod_behaviour_structure {
    /** Common sessions */
    ...
    /** @var int behaviour instance identifier */
    public $id;

    /** @var string behaviour activity name */
    public $name;

    /** @var float number (10, 5) unsigned, the maximum grade for behaviour */
    public $grade;

    /** @var int last time behaviour was modified - used for global search */
    public $timemodified;
    ...
    /**
     * Initializes the behaviour API instance using the data from DB
     *
     * Makes deep copy of all passed records properties. Replaces integer $course attribute
     * with a full database record (course should not be stored in instances table anyway).
     *
     * @param stdClass $dbrecord Attandance instance data from {behaviour} table
     * @param stdClass $cm       Course module record as returned by {@see get_coursemodule_from_id()}
     * @param stdClass $course   Course record from {course} table
     * @param stdClass $context  The context of the behaviour instance
    ...
    public function __construct(stdClass $dbrecord, stdClass $cm, stdClass $course, stdClass $context=null, $pageparams=null) {
        ...
        foreach ($dbrecord as $field => $value) {
            if (property_exists('mod_behaviour_structure', $field)) {
                $this->{$field} = $value;
            } else {
                throw new coding_exception('The behaviour table has a field with no property in the behaviour class');
            }
        }
        $this->cm           = $cm;
        if (empty($this->cmid)) {
        ...
        if (isset($pageparams->showextrauserdetails) && $pageparams->showextrauserdetails != $this->showextrauserdetails) {
            $DB->set_field('behaviour', 'showextrauserdetails', $pageparams->showextrauserdetails, array('id' => $this->id));
        }
        if (isset($pageparams->showsessiondetails) && $pageparams->showsessiondetails != $this->showsessiondetails) {
            $DB->set_field('behaviour', 'showsessiondetails', $pageparams->showsessiondetails, array('id' => $this->id));
        }
        if (isset($pageparams->sessiondetailspos) && $pageparams->sessiondetailspos != $this->sessiondetailspos) {
            $DB->set_field('behaviour', 'sessiondetailspos', $pageparams->sessiondetailspos, array('id' => $this->id));
        }
    }
    ...
    /**
     * Returns current sessions for this behaviour
     *
     * Fetches data from {behaviour_sessions}
     *
    ...
    /**
     * Returns today sessions for this behaviour
     *
     * Fetches data from {behaviour_sessions}
     *
     * @return array of records or an empty array
    ...
    /**
     * Returns today sessions suitable for copying behaviour log
     *
     * Fetches data from {behaviour_sessions}
     * @param stdClass $sess
     * @return array of records or an empty array
    ...
    public function get_today_sessions_for_copy($sess) : array {
        ...
        $sql = "SELECT *
                 ...
                 WHERE sessdate >= :start AND sessdate <= :end AND
                       ...
                       lasttaken > 0 AND behaviourid = :aid";
        $params = array(
            'start'     => $start,
            'end'       => $sess->sessdate,
    ...
    /**
     * Returns count of hidden sessions for this behaviour
     *
     * Fetches data from {behaviour_sessions}
     *
    ...
    public function get_hidden_sessions_count() : int {
        ...
        $where = "behaviourid = :aid AND sessdate < :csdate";
        $params = array(
            'aid'   => $this->id,
            'csdate' => $this->course->startdate);

        return $DB->count_records_select('behaviour_sessions', $where, $params);
    }

    /**
     * Returns the hidden sessions for this behaviour
     *
     * Fetches data from {behaviour_sessions}
     *
    ...
    public function get_hidden_sessions() : array {
        ...
        return $DB->get_records_select('behaviour_sessions', $where, $params);
    }
    ...
    public function get_filtered_sessions() : array {
        ...
        if ($this->pageparams->startdate && $this->pageparams->enddate) {
            $where = "behaviourid = :aid AND sessdate >= :csdate AND sessdate >= :sdate AND sessdate < :edate";
        } else if ($this->pageparams->enddate) {
            $where = "behaviourid = :aid AND sessdate >= :csdate AND sessdate < :edate";
        } else {
            $where = "behaviourid = :aid AND sessdate >= :csdate";
        }

        if ($this->pageparams->get_current_sesstype() > mod_behaviour_page_with_filter_controls::SESSTYPE_ALL) {
        ...
        $sessions = $DB->get_records_select('behaviour_sessions', $where, $params, 'sessdate asc');
        $statussetmaxpoints = behaviour_get_statusset_maxpoints($this->get_statuses(true, true));
        foreach ($sessions as $sess) {
            if (empty($sess->description)) {
                ...
                $sess->description = file_rewrite_pluginfile_urls($sess->description,
                    'pluginfile.php', $this->context->id, 'mod_behaviour', 'session', $sess->id);
            }
     ...
     * Get manage url.
     * @param array $params
     * @return moodle_url of manage.php for behaviour instance
     */
    public function url_manage($params=array()) : moodle_url {
        $params = array_merge(array('id' => $this->cm->id), $params);
        return new moodle_url('/mod/behaviour/manage.php', $params);
    }

    /**
     * Get manage temp users url.
     * @param array $params optional
     * @return moodle_url of tempusers.php for behaviour instance
     */
    public function url_managetemp($params=array()) : moodle_url {
        $params = array_merge(array('id' => $this->cm->id), $params);
        return new moodle_url('/mod/behaviour/tempusers.php', $params);
    }
     ...
     *
     * @param array $params optional
     * @return moodle_url of tempdelete.php for behaviour instance
     */
    public function url_tempdelete($params=array()) : moodle_url {
        $params = array_merge(array('id' => $this->cm->id, 'action' => 'delete'), $params);
        return new moodle_url('/mod/behaviour/tempedit.php', $params);
    }
     ...
     *
     * @param array $params optional
     * @return moodle_url of tempedit.php for behaviour instance
     */
    ...
    public function url_tempmerge($params=array()) : moodle_url {
        $params = array_merge(array('id' => $this->cm->id), $params);
        return new moodle_url('/mod/behaviour/tempmerge.php', $params);
    }

    /**
     * Get url for sessions.
     * @param array $params
     * @return moodle_url of sessions.php for behaviour instance
     */
    public function url_sessions($params=array()) : moodle_url {
        $params = array_merge(array('id' => $this->cm->id), $params);
        return new moodle_url('/mod/behaviour/sessions.php', $params);
    }

    /**
     * Get url for report.
     * @param array $params
     * @return moodle_url of report.php for behaviour instance
     */
    public function url_report($params=array()) : moodle_url {
        $params = array_merge(array('id' => $this->cm->id), $params);
        return new moodle_url('/mod/behaviour/report.php', $params);
    }
    ...
    public function url_absentee($params=array()) : moodle_url {
        $params = array_merge(array('id' => $this->cm->id), $params);
        return new moodle_url('/mod/behaviour/absentee.php', $params);
    }

    /**
     * Get url for import.
     *
     * @return moodle_url of import.php for behaviour instance
     */
    public function url_import() : moodle_url {
        $params = ['id' => $this->cm->id];
        return new moodle_url('/mod/behaviour/import.php', $params);
    }

    /**
     * Get url for export.
     *
     * @return moodle_url of export.php for behaviour instance
     */
    public function url_export() : moodle_url {
        $params = array('id' => $this->cm->id);
        return new moodle_url('/mod/behaviour/export.php', $params);
    }

    /**
     * Get preferences url
     * @param array $params
     * @return moodle_url of attsettings.php for behaviour instance
     */
    public function url_preferences($params=array()) : moodle_url {
        ...
        $params = array_merge(array('id' => $this->cm->id), $params);
        return new moodle_url('/mod/behaviour/preferences.php', $params);
    }
    ...
    public function url_warnings($params=array()) : moodle_url {
        ...
        $params = array_merge(array('id' => $this->cm->id), $params);
        return new moodle_url('/mod/behaviour/warnings.php', $params);
    }

    /**
     * Get take url.
     * @param array $params
     * @return moodle_url of behaviours.php for behaviour instance
     */
    public function url_take($params=array()) : moodle_url {
        $params = array_merge(array('id' => $this->cm->id), $params);
        return new moodle_url('/mod/behaviour/take.php', $params);
    }
    ...
    public function url_view($params=array()) : moodle_url {
        $params = array_merge(array('id' => $this->cm->id), $params);
        return new moodle_url('/mod/behaviour/view.php', $params);
    }
    ...
    public function add_session($sess) : int {
        ...
        $sess->id = $DB->insert_record('behaviour_sessions', $sess);
        $description = file_save_draft_area_files($sess->descriptionitemid,
            $this->context->id, 'mod_behaviour', 'session', $sess->id,
            array('subdirs' => false, 'maxfiles' => -1, 'maxbytes' => 0),
            $sess->description);
        $DB->set_field('behaviour_sessions', 'description', $description, array('id' => $sess->id));

        $sess->caleventid = 0;
        behaviour_create_calendar_event($sess);

        $infoarray = array();
        $infoarray[] = mod_behaviour_construct_session_full_date_time($sess->sessdate, $sess->duration);

        // Trigger a session added event.
        $event = \mod_behaviour\event\session_added::create(array(
            'objectid' => $this->id,
        ...
        $event->add_record_snapshot('behaviour_sessions', $sess);
        $event->trigger();
    ...
    public function update_session_from_form_data($formdata, $sessionid) {
        ...
        $description = file_save_draft_area_files($formdata->sdescription['itemid'],
            $this->context->id, 'mod_behaviour', 'session', $sessionid,
        ...
        $DB->update_record('behaviour_sessions', $sess);

        if (empty($sess->caleventid)) {
             // This shouldn't really happen, but just in case to prevent fatal error.
            behaviour_create_calendar_event($sess);
        } else {
            behaviour_update_calendar_event($sess);
        }

        $info = mod_behaviour_construct_session_full_date_time($sess->sessdate, $sess->duration);
        $event = \mod_behaviour\event\session_updated::create(array(
            'objectid' => $this->id,
            'context' => $this->context,
            'other' => array('info' => $info, 'sessionid' => $sessionid,
                'action' => mod_behaviour_sessions_page_params::ACTION_UPDATE)));
        $event->add_record_snapshot('course_modules', $this->cm);
        $event->add_record_snapshot('behaviour_sessions', $sess);
        $event->trigger();
    }
    ...
    /**
     * Used to record behaviour submitted by the student.
     *
    ...
    public function take_from_student($mformdata) : bool {
        ...
        $logid = $DB->insert_record('behaviour_log', $record, false);
        $record->id = $logid;
         ...
         * need to set grouptype to 0 to allow take behaviour page to be called
        ...
        // Log the change.
        $event = \mod_behaviour\event\behaviour_taken_by_student::create(array(
            'objectid' => $this->id,
            'context' => $this->context,
            'other' => $params));
        $event->add_record_snapshot('course_modules', $this->cm);
        $event->add_record_snapshot('behaviour_sessions', $session);
        $event->add_record_snapshot('behaviour_log', $record);
        $event->trigger();
        ...
        foreach ($formdata as $key => $value) {
            ...
            if (substr($key, 0, 7) == 'remarks') {
                ...
                if (!(is_numeric($sid))) { // Sanity check on $sid.
                    throw new moodle_exception('nonnumericid', 'behaviour');
                }
                $sesslog[$sid] = new stdClass();
                $sesslog[$sid]->studentid = $sid; // We check is_numeric on this above.
                $sesslog[$sid]->statusid = implode(',', (array)$formdata['user' . $sid]);
    ...
    /**
     * Helper function to save behaviour and trigger events.
     *
     * @param array $sesslog
        ...
        foreach ($sesslog as $log) {
            ...
            if (!empty($log->statusid) || !empty($log->remarks)) {
                if (array_key_exists($log->studentid, $dbsesslog)) {
                    ...
                    $DB->insert_record('behaviour_log', $log, false);
                }
            }
        }
    ...
    public function get_users($groupid = 0, $page = 1) : array {
        ...
        if ($page) {
            ...
            if (!empty($this->cm->groupingid)) {
                ...
                $users = get_users_by_capability($this->context, 'mod/behaviour:canbelisted',
                    $userfields,
                    $orderby, $startusers, $usersperpage, $groups,
                    '', false, true);
            } else {
                $startusers = ($page - 1) * $usersperpage;
                $users = get_enrolled_users($this->context, 'mod/behaviour:canbelisted', $groupid, $userfields,
            ...
            if (!empty($this->cm->groupingid)) {
                ...
                $users = get_enrolled_users($this->context, 'mod/behaviour:canbelisted', $groupid, $userfields, $orderby);
            }
        }
        ...
        $tempusers = $DB->get_records('behaviour_tempusers', array('courseid' => $this->course->id));
    ...
    public function get_user($userid) {
        ...
        // Look for 'temporary' users and return their details from the behaviour_tempusers table.
        if ($user->idnumber == 'tempghost') {
            $tempuser = $DB->get_record('behaviour_tempusers', array('studentid' => $userid), '*', MUST_EXIST);
    ...
    public function get_statuses($onlyvisible = true, $allsets = false) : array {
        if (!isset($this->statuses)) {
            ...
            $this->statuses = behaviour_get_statuses($this->id, $onlyvisible, $statusset);
            $this->allstatuses = behaviour_get_statuses($this->id, $onlyvisible);
        }
    ...
    public function get_session_info($sessionid) {
        ...
        if (!array_key_exists($sessionid, $this->sessioninfo)) {
            $this->sessioninfo[$sessionid] = $DB->get_record('behaviour_sessions', array('id' => $sessionid));
        }
        if (empty($this->sessioninfo[$sessionid]->description)) {
            $this->sessioninfo[$sessionid]->description = get_string('nodescription', 'behaviour');
        } else {
            $this->sessioninfo[$sessionid]->description = file_rewrite_pluginfile_urls($this->sessioninfo[$sessionid]->description,
                'pluginfile.php', $this->context->id, 'mod_behaviour', 'session', $this->sessioninfo[$sessionid]->id);
        }
    ...
    public function get_sessions_info($sessionids) : array {
        ...
        $sessions = $DB->get_records_select('behaviour_sessions', "id $sql", $params, 'sessdate asc');
    ...
    public function get_session_log($sessionid) : array {
        ...
        return $DB->get_records('behaviour_log', array('sessionid' => $sessionid), '', 'studentid,statusid,remarks,id,statusset');
    }
    ...
    public function update_users_grade($userids) {
        behaviour_update_users_grade($this, $userids);
    }
    ...
    public function get_user_filtered_sessions_log($userid) : array {
        ...
        if ($this->pageparams->startdate && $this->pageparams->enddate) {
            $where = "ats.behaviourid = :aid AND ats.sessdate >= :csdate AND
                      ats.sessdate >= :sdate AND ats.sessdate < :edate";
        } else {
            $where = "ats.behaviourid = :aid AND ats.sessdate >= :csdate";
        }
        if ($this->get_group_mode()) {
            $sql = "SELECT ats.id, ats.sessdate, ats.groupid, al.statusid, al.remarks,
                  ...
                  JOIN {behaviour_log} al ON ats.id = al.sessionid AND al.studentid = :uid
    ...
    public function get_user_filtered_sessions_log_extended($userid) : array {
        ...
        if ($this->pageparams->startdate && $this->pageparams->enddate) {
            ...
            $where = "ats.behaviourid = :aid AND ats.sessdate >= :csdate AND ats.groupid $gsql";
        }
        $sql = "SELECT $id, ats.id, ats.groupid, ats.sessdate, ats.duration, ats.description, ats.statusset,
                       al.statusid, al.remarks, ats.studentscanmark, ats.autoassignstatus,
                       ats.preventsharedip, ats.preventsharediptime, ats.rotateqrcode,
    ...
    public function delete_sessions($sessionsids) {
        ...
        $DB->delete_records_list('behaviour_sessions', 'id', $sessionsids);
        $event = \mod_behaviour\event\session_deleted::create(array(
            'objectid' => $this->id,
            'context' => $this->context,
    ...
    public function update_sessions_duration($sessionsids, $duration) {
        ...
        $now = time();
        $sessions = $DB->get_recordset_list('behaviour_sessions', 'id', $sessionsids);
        foreach ($sessions as $sess) {
            ...
            $event = \mod_behaviour\event\session_duration_updated::create(array(
                'objectid' => $this->id,
                'context' => $this->context,
                'other' => array('info' => implode(', ', $sessionsids))));
            $event->add_record_snapshot('course_modules', $this->cm);
    ...
    public static function check_existing_email($email, $tempuserid = 0) {
        ...
        if ($tempuser = $DB->get_record('behaviour_tempusers', array('email' => $email), 'id')) {
            if ($tempuser->id != $tempuserid) {
                return get_string('tempexists', 'behaviour');
            }
        }
        if ($DB->record_exists('user', array('email' => $email))) {
            return get_string('userexists', 'behaviour');
        }
        ...
        if ($time > $session->sessdate + $duration) {
            ...
            foreach ($statuses as $status) {
                ...
                if (empty($status->studentavailability) && ($session->sessdate + $duration >= $time) &&
                    !empty(get_config('behaviour', 'automark_useempty'))) {
        ...
        if (!isset($this->lowgradethreshold)) {
            ...
            if ($this->grade > 0) {
                $gradeitem = grade_item::fetch(array('courseid' => $this->course->id, 'itemtype' => 'mod',
                    'itemmodule' => 'behaviour', 'iteminstance' => $this->id, 'itemnumber' => 0));
