...
class mod_behaviour_summary {
     ...
     *
     * @param int $behaviourid instance identifier
     * @param array $userids user instances identifier
     * @param int $startdate behaviour sessions startdate
     * @param int $enddate behaviour sessions enddate
     */
    public function __construct($behaviourid, $userids=array(), $startdate = '', $enddate = '') {
    ...
    /**
     * Returns true if the corresponding behaviour instance is currently configure to work with grades (points)
     *
     * @return boolean
     */
    ...
    /**
     * Returns the groupmode of the corresponding behaviour instance
     *
     * @return int
     */
    ...
    public function get_user_taken_sessions_percentages() {
        ...
        foreach ($this->userspoints as $userid => $userpoints) {
            $percentages[$userid] = behaviour_calc_fraction($userpoints->points, $userpoints->maxpoints);
        }
    ...
    public function get_taken_sessions_summary_for($userid) {
        ...
        $usersummary->takensessionspercentage = behaviour_calc_fraction($usersummary->takensessionspoints,
    ...
    public function get_all_sessions_summary_for($userid) {
        ...
        $usersummary->allsessionspercentage = behaviour_calc_fraction($usersummary->takensessionspoints,
        ...
        $usersummary->maxpossiblepercentage = behaviour_calc_fraction(($usersummary->takensessionspoints + $deltapoints),
                                                                       $usersummary->allsessionsmaxpoints);
    ...
    private function compute_users_points($userids=array(), $startdate = '', $enddate = '') {
        ...
        list($this->course, $cm) = get_course_and_cm_from_instance($this->behaviourid, 'behaviour');
        $this->groupmode = $cm->effectivegroupmode;

        $params = array(
            'attid'      => $this->behaviourid,
            'attid2'     => $this->behaviourid,
    ...
    private function compute_users_taken_sessions_by_acronym($userids=array(), $startdate = '', $enddate = '') {
        ...
        $sql = "SELECT atl.studentid AS userid, sts.setnumber, sts.acronym, COUNT(*) AS numtakensessions
                  FROM {behaviour_sessions} ats
                  JOIN {behaviour_log} atl ON (atl.sessionid = ats.id)
                  JOIN {behaviour_statuses} sts
                    ON (sts.behaviourid = ats.behaviourid AND
