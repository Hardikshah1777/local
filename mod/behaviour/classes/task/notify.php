...
class notify extends \core\task\scheduled_task {
    ...
    public function get_name() {
        ...
        return get_string('notifytask', 'mod_behaviour');
    }
    ...
    public function execute() {
        ...
        // Get records for behaviour sessions that have been updated since last time this task ran.
        // Note: this returns all users for these sessions - even if the users behaviour wasn't changed
        // since last time we ran, before sending a notification we check to see if the users have
        // updated behaviour logs since last time they were notified.
        $records = behaviour_get_users_to_notify(array(), $orderby, true);
        ...
        foreach ($records as $record) {
            ...
            if (!empty($record->emailuser)) {
                // Only send one warning to this user from each behaviour in this run.
                ...
                if (empty($sentnotifications[$record->userid]) || !in_array($record->aid, $sentnotifications[$record->userid])) {
                    ...
                    // behaviour updated since the last time the notification was sent.
                    if (!empty($record->timesent)) {
                        $sql = "SELECT *
                              FROM {behaviour_log} l
                              JOIN {behaviour_sessions} s ON s.id = l.sessionid
                             WHERE s.behaviourid = ? AND studentid = ? AND timetaken > ?";
                    ...
                    $record = behaviour_template_variables($record);
            ...
            // Only send one warning to this user from each behaviour in this run. - flag any higher percent notifications as sent.
            ...
            if (!empty($record->thirdpartyemails)) {
                ...
                foreach ($sendto as $senduser) {
                    ...
                    if (has_capability('mod/behaviour:warningemails', $context, $senduser)) {
                        ...
                        if (!isset($thirdpartynotifications[$senduser][$record->aid . '_' . $record->userid])) {
                            $thirdpartynotifications[$senduser][$record->aid . '_' . $record->userid]
                                = get_string('thirdpartyemailtext', 'behaviour', $record);
                        }
                    } else {
            ...
            $notify->timesent = $now;
            $DB->insert_record('behaviour_warning_done', $notify);
        }
        ...
        if (!empty($thirdpartynotifications)) {
            foreach ($thirdpartynotifications as $sendid => $notifications) {
                ...
                if (empty($user) || !empty($user->deleted)) {
                    ...
                    $warnings = $DB->get_records_list('behaviour_warning', 'id', $thirdpartyusers[$sendid]);
                    if (!empty($warnings)) {
                        behaviour_remove_user_from_thirdpartyemails($warnings, $sendid);
                    }
                ...
                $emailcontent .= "\n\n".get_string('thirdpartyemailtextfooter', 'behaviour');
                $emailcontent = format_text($emailcontent);
                $emailsubject = get_string('thirdpartyemailsubject', 'behaviour');
