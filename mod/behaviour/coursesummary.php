...
// Check permissions.
require_capability('mod/behaviour:viewsummaryreports', $context);

$exportfilename = 'behaviourcoursesummary.csv';

$PAGE->set_url('/mod/behaviour/coursesummary.php', array('category' => $category));

$PAGE->set_heading($SITE->fullname);

$table = new flexible_table('behaviourcoursesummary');
$table->define_baseurl($PAGE->url);

if (!$table->is_downloading($download, $exportfilename)) {
    echo $OUTPUT->header();
    $heading = get_string('coursesummary', 'mod_behaviour');
    if (!empty($category)) {
    ...
    if ($admin) {
        ...
        $tabmenu = behaviour_print_settings_tabs('coursesummary');
        echo $tabmenu;
    }
    $url = new moodle_url('/mod/behaviour/coursesummary.php', array('category' => $category, 'fromcourse' => $fromcourse));

    if ($admin) {
        $options = core_course_category::make_categories_list('mod/behaviour:viewsummaryreports');
...
$records = behaviour_course_users_points($courses, $orderby);
