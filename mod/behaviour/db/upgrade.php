...
/**
 * upgrade this behaviour instance - this function could be skipped but it will be needed later
 * @param int $oldversion The old version of the behaviour module
 * @return bool
 */
function xmldb_behaviour_upgrade($oldversion=0) {
    ...
    if ($oldversion < 2014112000) {
        $table = new xmldb_table('behaviour_sessions');
        ...
        upgrade_mod_savepoint(true, 2014112000, 'behaviour');
    }

    if ($oldversion < 2014112001) {
        // Replace values that reference old module "attforblock" to "behaviour".
        $sql = "UPDATE {grade_items}
                   SET itemmodule = 'behaviour'
        ...
        foreach ($rs as $cap) {
            $renamedcapability = str_replace('mod/attforblock', 'mod/behaviour', $cap->capability);
        ...
        upgrade_mod_savepoint(true, 2014112001, 'behaviour');
    }

    if ($oldversion < 2015040501) {
        // Define table behaviour_tempusers to be created.
        $table = new xmldb_table('behaviour_tempusers');

        // Adding fields to table behaviour_tempusers.
        ...
        // Adding keys to table behaviour_tempusers.
        $table->add_key('primary', XMLDB_KEY_PRIMARY, array('id'));

        // Conditionally launch create table for behaviour_tempusers.
        ...
        upgrade_mod_savepoint(true, 2015040501, 'behaviour');
    }

    if ($oldversion < 2015040502) {

        // Define field setnumber to be added to behaviour_statuses.
        $table = new xmldb_table('behaviour_statuses');
        ...
        // Define field statusset to be added to behaviour_sessions.
        $table = new xmldb_table('behaviour_sessions');
        ...
        // Behaviour savepoint reached.
        upgrade_mod_savepoint(true, 2015040502, 'behaviour');
    }

    if ($oldversion < 2015040503) {

        // Changing type of field grade on table behaviour_statuses to number.
        $table = new xmldb_table('behaviour_statuses');
        ...
        // behaviour savepoint reached.
        upgrade_mod_savepoint(true, 2015040503, 'behaviour');
    }
    ...
    if ($oldversion < 2016052202) {
        ...
        behaviour_upgrade_create_calendar_events();

        // behaviour savepoint reached.
        upgrade_mod_savepoint(true, 2016052202, 'behaviour');
    }

    if ($oldversion < 2016082900) {

        // Define field timemodified to be added to behaviour.
        $table = new xmldb_table('behaviour');
        ...
        // behaviour savepoint reached.
        upgrade_mod_savepoint(true, 2016082900, 'behaviour');
    }
    if ($oldversion < 2016112100) {
        ...
        upgrade_mod_savepoint(true, 2016112100, 'behaviour');
    }
    ...
    if ($oldversion < 2016121300) {
        ...
        upgrade_mod_savepoint(true, 2016121300, 'behaviour');
    }
    ...
    if ($oldversion < 2017020700) {
        ...
        // behaviour savepoint reached.
        upgrade_mod_savepoint(true, 2017020700, 'behaviour');
    }
    ...
    if ($oldversion < 2017042800) {
        ...
        upgrade_mod_savepoint(true, 2017042800, 'behaviour');
    }

    if ($oldversion < 2017051101) {

        // Define field studentavailability to be added to behaviour_statuses.
        $table = new xmldb_table('behaviour_statuses');
        ...
        // behaviour savepoint reached.
        upgrade_mod_savepoint(true, 2017051101, 'behaviour');
    }
    ...
    if ($oldversion < 2017051103) {
        ...
        upgrade_mod_savepoint(true, 2017051103, 'behaviour');
    }

    if ($oldversion < 2017051104) {
        // The meaning of the subnet in the behaviour table has changed - it is now the "default" value - find all existing
        // behaviour with subnet set and set the session subnet for these.
        $behaviours = $DB->get_recordset_select('behaviour', 'subnet IS NOT NULL');
        foreach ($behaviours as $behaviour) {
            if (!empty($behaviour->subnet)) {
                // Get all sessions for this behaviour.
                $sessions = $DB->get_recordset('behaviour_sessions', array('behaviourid' => $behaviour->id));
                foreach ($sessions as $session) {
                    $session->subnet = $behaviour->subnet;
        ...
        upgrade_mod_savepoint(true, 2017051104, 'behaviour');
    }

    if ($oldversion < 2017051900) {
        // Define field setunmarked to be added to behaviour_statuses.
        $table = new xmldb_table('behaviour_statuses');
        ...
        // behaviour savepoint reached.
        upgrade_mod_savepoint(true, 2017051900, 'behaviour');
    }
    ...
    if ($oldversion < 2017052201) {
        ...
        // behaviour savepoint reached.
        upgrade_mod_savepoint(true, 2017052201, 'behaviour');
    }

    if ($oldversion < 2017060900) {
        // Automark values changed.
        $default = get_config('behaviour', 'automark_default');
        if (!empty($default)) { // Change default if set.
            set_config('automark_default', 2, 'behaviour');
        }
        // Update any sessions set to use automark = 1.
        $sql = "UPDATE {behaviour_sessions} SET automark = 2 WHERE automark = 1";
        $DB->execute($sql);

        // Update automarkcompleted to 2 if already complete.
        $sql = "UPDATE {behaviour_sessions} SET automarkcompleted = 2 WHERE automarkcompleted = 1";
        $DB->execute($sql);

        upgrade_mod_savepoint(true, 2017060900, 'behaviour');
    }

    if ($oldversion < 2017062000) {

        // Define table behaviour_warning_done to be created.
        $table = new xmldb_table('behaviour_warning_done');

        // Adding fields to table behaviour_warning_done.
        $table->add_field('id', XMLDB_TYPE_INTEGER, '10', null, XMLDB_NOTNULL, XMLDB_SEQUENCE, null);
        ...
        // Adding keys to table behaviour_warning_done.
        $table->add_key('primary', XMLDB_KEY_PRIMARY, array('id'));

        // Adding indexes to table behaviour_warning_done.
        $table->add_index('notifyid_userid', XMLDB_INDEX_UNIQUE, array('notifyid', 'userid'));

        // Conditionally launch create table for behaviour_warning_done.
        if (!$dbman->table_exists($table)) {
            $dbman->create_table($table);
        }

        // behaviour savepoint reached.
        upgrade_mod_savepoint(true, 2017062000, 'behaviour');
    }

    if ($oldversion < 2017071305) {

        // Define table behaviour_warning to be created.
        $table = new xmldb_table('behaviour_warning');

        if (!$dbman->table_exists($table)) {
            // Adding fields to table behaviour_warning.
            $table->add_field('id', XMLDB_TYPE_INTEGER, '10', null, XMLDB_NOTNULL, XMLDB_SEQUENCE, null);
            ...
            // Adding keys to table behaviour_warning.
            $table->add_key('primary', XMLDB_KEY_PRIMARY, array('id'));
            $table->add_key('level_id', XMLDB_KEY_UNIQUE, array('idnumber', 'warningpercent', 'warnafter'));

            // Conditionally launch create table for behaviour_warning.
            $dbman->create_table($table);

        } else {
            // Key definition is probably incorrect so fix it - drop_key dml function doesn't seem to work.
            $indexes = $DB->get_indexes('behaviour_warning');
            foreach ($indexes as $name => $index) {
                if ($DB->get_dbfamily() === 'mysql') {
                    $DB->execute("ALTER TABLE {behaviour_warning} DROP INDEX ". $name);
                } else {
        ...
        // behaviour savepoint reached.
        upgrade_mod_savepoint(true, 2017071305, 'behaviour');
    }
    ...
    if ($oldversion < 2017071800) {
        ...
        // behaviour savepoint reached.
        upgrade_mod_savepoint(true, 2017071800, 'behaviour');
    }
    ...
    if ($oldversion < 2017071802) {
        ...
        // behaviour savepoint reached.
        upgrade_mod_savepoint(true, 2017071802, 'behaviour');
    }

    if ($oldversion < 2017082200) {
        // Warnings idnumber field should use behaviourid instead of cmid.
        ...
        foreach ($warnings as $warning) {
            if (!empty($warning->idnumber) && !empty($idnumbers[$warning->idnumber])) {
                ...
                $DB->update_record("behaviour_warning", $warning);
            }
        }
        $warnings->close();

        // behaviour savepoint reached.
        upgrade_mod_savepoint(true, 2017082200, 'behaviour');
    }
    ...
    if ($oldversion < 2017120700) {
        ...
        upgrade_mod_savepoint(true, 2017120700, 'behaviour');
    }
    ...
    if ($oldversion < 2017120801) {
        ...
        upgrade_mod_savepoint(true, 2017120801, 'behaviour');
    }
    ...
    if ($oldversion < 2018022204) {
        ...
        upgrade_mod_savepoint(true, 2018022204, 'behaviour');
    }
    ...
    if ($oldversion < 2018050100) {
        ...
        $table = new xmldb_table('behaviour_log');
        ...
        upgrade_mod_savepoint(true, 2018050100, 'behaviour');
    }
    ...
    if ($oldversion < 2018072700) {
        ...
        if (!$dbman->field_exists($table, $field)) {
            ...
            if (empty(get_config('behaviour', 'enablecalendar'))) {
                ...
                $DB->execute("UPDATE {behaviour_sessions} set calendarevent = 0");
            }
        }
        upgrade_mod_savepoint(true, 2018072700, 'behaviour');
    }
    ...
    if ($oldversion < 2018082605) {
        ...
        upgrade_mod_savepoint(true, 2018082605, 'behaviour');
    }

    if ($oldversion < 2019012500) {

        // Changing precision of field statusset on table behaviour_log to (1333).
        $table = new xmldb_table('behaviour_log');
        ...
        // behaviour savepoint reached.
        upgrade_mod_savepoint(true, 2019012500, 'behaviour');
    }
    ...
    if ($oldversion < 2019061800) {
        ...
        // behaviour savepoint reached.
        upgrade_mod_savepoint(true, 2019061800, 'behaviour');
    }
    ...
    if ($oldversion < 2019062000) {
        ...
        // behaviour savepoint reached.
        upgrade_mod_savepoint(true, 2019062000, 'behaviour');
    }

    if ($oldversion < 2019062200) {

        // Define table behaviour_rotate_passwords to be created.
        $table = new xmldb_table('behaviour_rotate_passwords');

        // Adding fields to table behaviour_rotate_passwords.
        $table->add_field('id', XMLDB_TYPE_INTEGER, '10', null, XMLDB_NOTNULL, XMLDB_SEQUENCE, null);
        $table->add_field('behaviourid', XMLDB_TYPE_INTEGER, '10', null, XMLDB_NOTNULL, null, null);
        ...
        // Adding keys to table behaviour_rotate_passwords.
        $table->add_key('id', XMLDB_KEY_PRIMARY, ['id']);

        // Conditionally launch create table for behaviour_rotate_passwords.
        if (!$dbman->table_exists($table)) {
            $dbman->create_table($table);
        }

        // Define field rotateqrcode to be added to behaviour_sessions.
        $table = new xmldb_table('behaviour_sessions');
        ...
        // Define field rotateqrcodesecret to be added to behaviour_sessions.
        $table = new xmldb_table('behaviour_sessions');
        ...
        // behaviour savepoint reached.
        upgrade_mod_savepoint(true, 2019062200, 'behaviour');
    ...
    if ($oldversion < 2020072900) {
        ...
        // behaviour savepoint reached.
        upgrade_mod_savepoint(true, 2020072900, 'behaviour');
    }

    if ($oldversion < 2021050700) {
        // Restore process sometimes creates orphan behaviour calendar events - clean them up.
        $sql = "modulename = 'behaviour' AND id NOT IN (SELECT caleventid
                                                           FROM {behaviour_sessions})";
        $DB->delete_records_select('event', $sql);

        // behaviour savepoint reached.
        upgrade_mod_savepoint(true, 2021050700, 'behaviour');
    }
    ...
    if ($oldversion < 2021082400) {
        ...
        // behaviour savepoint reached.
        upgrade_mod_savepoint(true, 2021082400, 'behaviour');
    }

    if ($oldversion < 2021082401) {

        // Changing the default of field automarkcmid on table behaviour_sessions to 0.
        $table = new xmldb_table('behaviour_sessions');
        ...
        // behaviour savepoint reached.
        upgrade_mod_savepoint(true, 2021082401, 'behaviour');
    }

    if ($oldversion < 2021082402) {

        // Changing the default of field ipaddress on table behaviour_log to .
        $table = new xmldb_table('behaviour_log');
        ...
        // behaviour savepoint reached.
        upgrade_mod_savepoint(true, 2021082402, 'behaviour');
    }
    ...
    if ($oldversion < 2021082600) {
        ...
        if ($DB->record_exists_select('behaviour_sessions', 'automark > 0')) {
            set_config('automark_useempty', '0', 'behaviour');
        }

        // behaviour savepoint reached.
        upgrade_mod_savepoint(true, 2021082600, 'behaviour');
    }

    if ($oldversion < 2022082900) {

        // Changing precision of field remarks on table behaviour_log to (1333).
        $table = new xmldb_table('behaviour_log');
        ...
        // behaviour savepoint reached.
        upgrade_mod_savepoint(true, 2022082900, 'behaviour');
    }

    if ($oldversion < 2022083100) {

        // Define field studentsearlyopentime to be added to behaviour_sessions.
        $table = new xmldb_table('behaviour_sessions');
        ...
        // behaviour savepoint reached.
        upgrade_mod_savepoint(true, 2022083100, 'behaviour');
    }
    if ($oldversion < 2022083105) {
        if (!empty($CFG->dbfamily) && $CFG->dbfamily == 'postgres') {
            $sql = 'DELETE FROM {behaviour_log}
            ...
            foreach ($records as $record) {
                $DB->delete_records('behaviour_log', ['id' => $record->id]);
            }
        } else if (!empty($CFG->dbfamily) && $CFG->dbfamily == 'mssql') {
            $sql = "DELETE {behaviour_log}
                    WHERE id NOT IN (
                        SELECT max(id)
                        FROM {behaviour_log}
                        GROUP BY sessionid, studentid, statusid
        ...
        // behaviour savepoint reached.
        upgrade_mod_savepoint(true, 2022083105, 'behaviour');
    }
