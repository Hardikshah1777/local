...
/**
 * Function to help upgrade old behaviour records and create calendar events.
 */
function behaviour_upgrade_create_calendar_events() {
    global $DB;

    $behaviours = $DB->get_records('behaviour', null, null, 'id, name, course');
    foreach ($behaviours as $att) {
        $sessionsdata = $DB->get_records('behaviour_sessions', array('behaviourid' => $att->id), null,
        ...
        foreach ($sessionsdata as $session) {
            ...
            $calevent->eventtype      = 'behaviour';
            $calevent->timemodified   = time();
            $calevent->modulename     = 'behaviour';
            ...
            $DB->set_field('behaviour_sessions', 'caleventid', $caleventid, array('id' => $session->id));
        }
    }
}
