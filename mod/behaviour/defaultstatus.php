...
$url = new moodle_url('/mod/behaviour/defaultstatus.php', array('statusid' => $statusid, 'action' => $action));
...
echo $OUTPUT->header();
echo $OUTPUT->heading(get_string('defaultstatus', 'mod_behaviour'));
$tabmenu = behaviour_print_settings_tabs('defaultstatus');
echo $tabmenu;
...
switch ($action) {
    case mod_behaviour_preferences_page_params::ACTION_ADD:
        ...
        // Default value uses setnumber/behaviourid = 0.
        $status = new stdClass();
        $status->behaviourid = 0;
        ...
        $status->setnumber = 0;
        behaviour_add_status($status);

        break;
    case mod_behaviour_preferences_page_params::ACTION_DELETE:
        $confirm    = optional_param('confirm', null, PARAM_INT);
        $statuses = behaviour_get_statuses(0, false);
        $status = $statuses[$statusid];

        if (isset($confirm)) {
            behaviour_remove_status($status);
            echo $OUTPUT->notification(get_string('statusdeleted', 'behaviour'), 'success');
            break;
        }
        ...
        $cancelurl = new moodle_url('/mod/behaviour/defaultstatus.php');
        $confirmurl = $url;
    ...
    case mod_behaviour_preferences_page_params::ACTION_HIDE:
        $statuses = behaviour_get_statuses(0, false);
        $status = $statuses[$statusid];
        behaviour_update_status($status, null, null, null, 0);
        break;
    case mod_behaviour_preferences_page_params::ACTION_SHOW:
        $statuses = behaviour_get_statuses(0, false);
        $status = $statuses[$statusid];
        behaviour_update_status($status, null, null, null, 1);
        break;
    case mod_behaviour_preferences_page_params::ACTION_SAVE:
        ...
        echo $OUTPUT->notification(get_string('eventstatusupdated', 'behaviour'), 'success');

        break;
}

$statuses = behaviour_get_statuses(0, false);
$prefdata = new mod_behaviour\output\default_statusset($statuses, $errors);
