...
require_capability('mod/behaviour:export', $context);
...
$PAGE->navbar->add(get_string('export', 'behaviour'));

$formparams = array('course' => $course, 'cm' => $cm, 'modcontext' => $context);
$mform = new mod_behaviour\form\export($att->url_export(), $formparams);
...
if ($formdata = $mform->get_data()) {
    ...
    $pageparams = new mod_behaviour_page_with_filter_controls();
    $pageparams->init($cm);
    $pageparams->page = 0;
    $pageparams->group = $formdata->group;
    $pageparams->set_current_sesstype($formdata->group ? $formdata->group : mod_behaviour_page_with_filter_controls::SESSTYPE_ALL);
    ...
    $reportdata = new mod_behaviour\output\report_data($att);
    if ($reportdata->users) {
        $filename = clean_filename($course->shortname.'_'.
            get_string('modulenameplural', 'behaviour').
        ...
        if (isset($formdata->ident)) {
            foreach (array_keys($formdata->ident) as $opt) {
                if ($opt == 'id') {
                    $data->tabhead[] = get_string('studentid', 'behaviour');
        ...
        if (count($reportdata->sessions) > 0) {
            foreach ($reportdata->sessions as $sess) {
                $text = userdate($sess->sessdate, get_string('strftimedmyhm', 'behaviour'));
                $text .= ' ';
                if (!empty($sess->groupid) && empty($reportdata->groups[$sess->groupid])) {
                    $text .= get_string('deletedgroup', 'behaviour');
                } else {
                    $text .= $sess->groupid ? $reportdata->groups[$sess->groupid]->name : get_string('commonsession', 'behaviour');
                }
            ...
            throw new moodle_exception('sessionsnotfound', 'mod_behaviour', $att->url_manage());
        }
        ...
        $data->tabhead[] = get_string('takensessions', 'behaviour');
        $data->tabhead[] = get_string('points', 'behaviour');
        $data->tabhead[] = get_string('percentage', 'behaviour');
        ...
        foreach ($reportdata->users as $user) {
            ...
            $cellsgenerator = new behaviour_user_sessions_cells_text_generator($reportdata, $user);
        ...
        if ($formdata->format === 'text') {
            behaviour_exporttocsv($data, $filename);
        } else {
            behaviour_exporttotableed($data, $filename, $formdata->format);
        }
        exit;
    } else {
        throw new moodle_exception('studentsnotfound', 'mod_behaviour', $att->url_manage());
    }
}
