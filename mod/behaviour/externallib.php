...
/**
 * Externallib.php file for behaviour plugin.
 *
...
require_once(dirname(__FILE__).'/classes/behaviour_webservices_handler.php');

/**
 * Class mod_behaviour_external
 * @copyright  2015 Caio Bressan Doneda
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class mod_behaviour_external extends external_api {

    /**
     * Describes the parameters for add_behaviour.
     *
     * @return external_function_parameters
     */
    public static function add_behaviour_parameters() {
        return new external_function_parameters(
            array(
                'courseid' => new external_value(PARAM_INT, 'course id'),
                'name' => new external_value(PARAM_TEXT, 'behaviour name'),
                'intro' => new external_value(PARAM_RAW, 'behaviour description', VALUE_DEFAULT, ''),
    ...
    /**
     * Adds behaviour instance to course.
     *
     * @param int $courseid
     * @param string $name
     * @param string $intro
     ...
     * @return array
     */
    public static function add_behaviour(int $courseid, $name, $intro, int $groupmode) {
        global $CFG, $DB;
        require_once($CFG->dirroot.'/course/modlib.php');

        $params = self::validate_parameters(self::add_behaviour_parameters(), array(
            'courseid' => $courseid,
            'name' => $name,
            'intro' => $intro,
            'groupmode' => $groupmode,
        ...
        // Verify permissions.
        list($module, $context) = can_add_moduleinfo($course, 'behaviour', 0);
        self::validate_context($context);
        require_capability('mod/behaviour:addinstance', $context);
        ...
        $moduleinfo->modulename = 'behaviour';
        ...
        return array('behaviourid' => $moduleinfo->instance);
    }

    /**
     * Describes add_behaviour return values.
     *
     * @return external_multiple_structure
     */
    public static function add_behaviour_returns() {
        return new external_single_structure(array(
            'behaviourid' => new external_value(PARAM_INT, 'instance id of the created behaviour'),
        ));
    }

    /**
     * Describes the parameters for remove_behaviour.
     *
     * @return external_function_parameters
     */
    public static function remove_behaviour_parameters() {
        return new external_function_parameters(
            array(
                'behaviourid' => new external_value(PARAM_INT, 'behaviour instance id'),
            )
        );
    }

    /**
     * Remove behaviour instance.
     *
     * @param int $behaviourid
     */
    public static function remove_behaviour(int $behaviourid) {
        $params = self::validate_parameters(self::remove_behaviour_parameters(), array(
            'behaviourid' => $behaviourid,
        ));

        $cm = get_coursemodule_from_instance('behaviour', $params['behaviourid'], 0, false, MUST_EXIST);

        // Check permissions.
        $context = context_module::instance($cm->id);
        self::validate_context($context);
        require_capability('mod/behaviour:managebehaviours', $context);

        // Delete behaviour instance.
        $result = behaviour_delete_instance($params['behaviourid']);
        rebuild_course_cache($cm->course, true);
        return $result;
    }

    /**
     * Describes remove_behaviour return values.
     *
     * @return external_value
     */
    public static function remove_behaviour_returns() {
        return new external_value(PARAM_BOOL, 'behaviour deletion result');
    }
    ...
    /**
     * Adds session to behaviour instance.
     *
     * @param int $behaviourid
     ...
     * @return array
     */
    public static function add_session(int $behaviourid, $description, int $sessiontime, int $duration, int $groupid,
        ...
        // Get behaviour.
        $behaviour = new mod_behaviour_structure($behaviour, $cm, $course, $context);
        ...
        $sess->absenteereport = get_config('behaviour', 'absenteereport_default');
        $sess->includeqrcode = 0;
        $sess->subnet = $behaviour->subnet;
        $sess->statusset = 0;
        $sess->groupid = $groupid;
        $sess->automarkcmid = 0;
        $sess->studentsearlyopentime = get_config('behaviour', 'studentsearlyopentime');

        $sessionid = $behaviour->add_session($sess);
    ...
    /**
     * Delete session from behaviour instance.
     *
     * @param int $sessionid
     * @return bool
     */
    public static function remove_session(int $sessionid) {
        ...
        $session = $DB->get_record('behaviour_sessions', array('id' => $params['sessionid']), '*', MUST_EXIST);
        $behaviour = $DB->get_record('behaviour', array('id' => $session->behaviourid), '*', MUST_EXIST);
        $cm = get_coursemodule_from_instance('behaviour', $behaviour->id, 0, false, MUST_EXIST);
        $course = $DB->get_record('course', array('id' => $cm->course), '*', MUST_EXIST);
        ...
        // Delete session.
        $behaviour->delete_sessions(array($sessionid));
        behaviour_update_users_grade($behaviour);
    ...
    public static function remove_session_returns() {
        return new external_value(PARAM_BOOL, 'behaviour session deletion result');
    }
    ...
    public static function get_courses_with_today_sessions($userid) {
        ...
        return behaviour_handler::get_courses_with_today_sessions($params['userid']);
    }

    /**
     * Get structure of an behaviour session.
     *
     * @return array
     */
    private static function get_session_structure() {
        $session = array('id' => new external_value(PARAM_INT, 'Session id.'),
                         'behaviourid' => new external_value(PARAM_INT, 'behaviour id.'),
    ...
    public static function get_courses_with_today_sessions_returns() {
        ...
        $behaviourinstances = array('name' => new external_value(PARAM_TEXT, 'Behaviour name.'),
        ...
        $courses = array('shortname' => new external_value(PARAM_TEXT, 'short name of a moodle course.'),
                         ...
                         'behaviour_instances' => new external_multiple_structure(
                                                   new external_single_structure($behaviourinstances)));
    ...
    public static function get_session($sessionid) {
        ...
        return behaviour_handler::get_session($sessionid);
    }
    ...
    public static function get_session_returns() {
        ...
        $behaviourlog = array('studentid' => new external_value(PARAM_INT, 'Student id.'),
        ...
        $session['behaviour_log'] = new external_multiple_structure(new external_single_structure($behaviourlog));
    ...
    public static function update_user_status($sessionid, $studentid, $takenbyid, $statusid, $statusset) {
        ...
        $session = $DB->get_record('behaviour_sessions', array('id' => $params['sessionid']), '*', MUST_EXIST);
        $cm = get_coursemodule_from_instance('behaviour', $session->behaviourid, 0, false, MUST_EXIST);
        ...
        if (!has_capability('mod/behaviour:takebehaviours', $context)) {
            list($canmark, $reason) = behaviour_can_student_mark($session);
            if (!$canmark) {
        ...
        return behaviour_handler::update_user_status($params['sessionid'], $params['studentid'], $params['takenbyid'],
    ...
    public static function get_sessions_parameters() {
        return new external_function_parameters(
                    array(
                        'behaviourid' => new external_value(PARAM_INT, 'Behaviour id.', VALUE_REQUIRED),
                    )
                );
    }
    ...
    public static function get_sessions($behaviourid) {
        ...
        return behaviour_handler::get_sessions($params['behaviourid']);
    }
}
