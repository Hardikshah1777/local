...
/**
 * Mark behaviour sessions using a csv import.
 *
...
$url = new moodle_url('/mod/behaviour/import/marksessions.php');
$PAGE->set_url($url);
...
if (optional_param('needsconfirm', 0, PARAM_BOOL)) {
    $form = new \mod_behaviour\form\import\marksessions($url->out(false), $formparams);
} else if (optional_param('confirm', 0, PARAM_BOOL)) {
    $importer = new \mod_behaviour\import\marksessions(null, $att, null, null, $importid);
    $formparams['importer'] = $importer;
    $form = new \mod_behaviour\form\import\marksessions_confirm(null, $formparams);
} else {
    $form = new \mod_behaviour\form\import\marksessions($url->out(false), $formparams);
}

if ($form->is_cancelled()) {
    redirect(new moodle_url('/mod/behaviour/take.php',
             array('id' => $cm->id,
             'sessionid' => $pageparams->sessionid,
             'grouptype' => $pageparams->grouptype)));
    return;
    ...
    if ($data->confirm) {
        ...
        $importer = new \mod_behaviour\import\marksessions(null, $att, null, null, $importid, $data, true);
        ...
        if ($error) {
            ...
            $url = new moodle_url('/mod/behaviour/manage.php', array('id' => $att->cmid));
            echo $output->continue_button($url);
            echo $output->footer();
            die();
        }
    } else {
        $text = $form->get_file_content('behaviourfile');
        $encoding = $data->encoding;
        $delimiter = $data->separator;
        $importer = new \mod_behaviour\import\marksessions($text, $att, $encoding, $delimiter, 0, null, true);
        $formparams['importer'] = $importer;
        $confirmform = new \mod_behaviour\form\import\marksessions_confirm(null, $formparams);
        $form = $confirmform;
        $pagetitle = get_string('confirmcolumnmappings', 'behaviour');
    }
}

// Output for the file upload form starts here.
echo $output->header();
echo $output->box(get_string('marksessionimportcsvhelp', 'behaviour'));
mod_behaviour_notifyqueue::show();
$form->display();
echo $output->footer();
