...
/**
 * local functions and constants for module behaviour
 *
...
define('BEHAVIOUR_AUTOMARK_DISABLED', 0);
define('BEHAVIOUR_AUTOMARK_ALL', 1);
define('BEHAVIOUR_AUTOMARK_CLOSE', 2);
define('BEHAVIOUR_AUTOMARK_ACTIVITYCOMPLETION', 3);


define('BEHAVIOUR_SHAREDIP_DISABLED', 0);
define('BEHAVIOUR_SHAREDIP_MINUTES', 1);
define('BEHAVIOUR_SHAREDIP_FORCE', 2);

// Max number of sessions available in the warnings set form to trigger warnings.
define('BEHAVIOUR_MAXWARNAFTER', 100);
 ...
 * @return array
 */
function behaviour_get_statuses($attid, $onlyvisible=true, $statusset = -1) {
    global $DB;
    ...
    if ($onlyvisible) {
        $statuses = $DB->get_records_select('behaviour_statuses', "behaviourid = :aid AND visible = 1 AND deleted = 0 $setsql",
                                            $params, 'setnumber ASC, grade DESC');
    } else {
        $statuses = $DB->get_records_select('behaviour_statuses', "behaviourid = :aid AND deleted = 0 $setsql",
                                            $params, 'setnumber ASC, grade DESC');
    }
 ...
 * @return string
 */
function behaviour_get_setname($attid, $statusset, $includevalues = true) {
    $statusname = get_string('statusset', 'mod_behaviour', $statusset + 1);
    if ($includevalues) {
        $statuses = behaviour_get_statuses($attid, true, $statusset);
        $statusesout = array();
 ...
 * @return array
 */
function behaviour_get_user_sessions_log_full($userid, $pageparams) {
    global $DB;
    ...
    // gm.userid not null => get unmarked behaviours for user's current groups
    ...
    $sql = "SELECT ats.id, ats.groupid, ats.sessdate, ats.duration, ats.description, ats.statusset,
                   ...
                   ats.behaviourid, att.name AS attname, att.course AS courseid, c.fullname AS cname
              FROM {behaviour_sessions} ats
              JOIN {behaviour} att
                ON att.id = ats.behaviourid
              JOIN {course} c
                ON att.course = c.id
         LEFT JOIN {behaviour_log} al
    ...
    foreach ($sessions as $sess) {
        if (empty($sess->description)) {
            $sess->description = get_string('nodescription', 'behaviour');
        } else {
            $modinfo = get_fast_modinfo($sess->courseid);
            $cmid = $modinfo->instances['behaviour'][$sess->behaviourid]->get_course_module_record()->id;
            $ctx = context_module::instance($cmid);
            $sess->description = format_text(file_rewrite_pluginfile_urls($sess->description,
            'pluginfile.php', $ctx->id, 'mod_behaviour', 'session', $sess->id));
        }
    }
...
/**
 * Get users courses and the relevant behaviours.
 *
 * @param int $userid
 * @return array
 */
function behaviour_get_user_courses_behaviours($userid) {
    global $DB;
 ...
 */
function behaviour_calc_fraction($part, $total) {
    if ($total == 0) {
        return 0;
    } else {
        return $part / $total;
 ...
 */
function behaviour_has_logs_for_status($statusid) {
    global $DB;
    return $DB->record_exists('behaviour_log', array('statusid' => $statusid));
}
 ...
 */
function behaviour_form_sessiondate_selector (MoodleQuickForm $mform) {

    $mform->addElement('date_selector', 'sessiondate', get_string('sessiondate', 'behaviour'));
    ...
    $sesendtime = array();
    if (!right_to_left()) {
        $sesendtime[] =& $mform->createElement('static', 'from', '', get_string('from', 'behaviour'));
        ...
        $sesendtime[] =& $mform->createElement('static', 'to', '', get_string('to', 'behaviour'));
    ...
    $mform->addGroup($sesendtime, 'sestime', get_string('time', 'behaviour'), array(' '), true);
}
 ...
 * @return int
 */
function behaviour_get_max_statusset($behaviourid) {
    global $DB;

    $max = $DB->get_field_sql('SELECT MAX(setnumber) FROM {behaviour_statuses} WHERE behaviourid = ? AND deleted = 0',
        array($behaviourid));
    if ($max) {
        return $max;
    }
 ...
 * @return array
 */
function behaviour_get_statusset_maxpoints($statuses) {
 ...
 * Update user grades
 *
 * @param mod_behaviour_structure|stdClass $behaviour
 * @param array $userids
 */
function behaviour_update_users_grade($behaviour, $userids=array()) {
    global $DB;

    if (empty($behaviour->grade)) {
        return false;
    }

    list($course, $cm) = get_course_and_cm_from_instance($behaviour->id, 'behaviour');

    $summary = new mod_behaviour_summary($behaviour->id, $userids);

    if (empty($userids)) {
        $context = context_module::instance($cm->id);
        $userids = array_keys(get_enrolled_users($context, 'mod/behaviour:canbelisted', 0, 'u.id'));
    }

    if ($behaviour->grade < 0) {
        $dbparams = array('id' => -($behaviour->grade));
        $scale = $DB->get_record('scale', $dbparams);
        $scalearray = explode(',', $scale->scale);
        $behaviourgrade = count($scalearray);
    } else {
        $behaviourgrade = $behaviour->grade;
    }
    ...
    foreach ($userids as $userid) {
        ...
        if ($summary->has_taken_sessions($userid)) {
            ...
            $grades[$userid]->rawgrade = $usersummary->takensessionspercentage * $behaviourgrade;
        } else {
            $grades[$userid]->rawgrade = null;
        }
    }

    return grade_update('mod/behaviour', $course->id, 'mod', 'behaviour', $behaviour->id, 0, $grades);
}

/**
 * Update grades for specified users for specified behaviour
 *
 * @param integer $behaviourid - the id of the behaviour to update
 * @param integer $grade - the value of the 'grade' property of the specified behaviour
 * @param array $userids - the userids of the users to be updated
 */
function behaviour_update_users_grades_by_id($behaviourid, $grade, $userids) {
    global $DB;
    ...
    list($course, $cm) = get_course_and_cm_from_instance($behaviourid, 'behaviour');

    $summary = new mod_behaviour_summary($behaviourid, $userids);
    ...
    return grade_update('mod/behaviour', $course->id, 'mod', 'behaviour', $behaviourid, 0, $grades);
}

/**
 * Add an behaviour status variable
 *
 * @param stdClass $status
 * @return bool
 */
function behaviour_add_status($status) {
    global $DB;
    ...
    if (!empty($status->acronym) && !empty($status->description)) {
        ...
        $id = $DB->insert_record('behaviour_statuses', $status);
        $status->id = $id;

        $event = \mod_behaviour\event\status_added::create(array(
            'objectid' => $status->behaviourid,
        ...
        $event->add_record_snapshot('behaviour_statuses', $status);
        $event->trigger();
        return true;
    } else {
        return false;
...
/**
 * Remove a status variable from an behaviour instance
 *
 * @param stdClass $status
 * @param stdClass $context
 * @param stdClass $cm
 */
function behaviour_remove_status($status, $context = null, $cm = null) {
    global $DB;
    if (empty($context)) {
        $context = context_system::instance();
    }
    $DB->set_field('behaviour_statuses', 'deleted', 1, array('id' => $status->id));
    $event = \mod_behaviour\event\status_removed::create(array(
        'objectid' => $status->id,
        'context' => $context,
        'other' => array(
...
/**
 * Update status variable for a particular behaviour module instance
 *
 * @param stdClass $status
 ...
 * @return array
 */
function behaviour_update_status($status, $acronym, $description, $grade, $visible,
    ...
    $DB->update_record('behaviour_statuses', $status);

    $event = \mod_behaviour\event\status_updated::create(array(
        'objectid' => $status->behaviourid,
        'context' => $context,
 ...
 * @return string
 */
function behaviour_random_string($length=6) {
 ...
 *
 * @param stdclass $sess the session record from behaviour_sessions.
 * @param boolean $log - if student cannot mark, generate log event.
 * @return array (boolean, string reason for failure)
 */
function behaviour_can_student_mark($sess, $log = true) {
    global $DB, $USER, $OUTPUT;
    $canmark = false;
    $reason = 'closed';
    $attconfig = get_config('behaviour');
    ...
    // Check if another student has marked behaviour from this IP address recently.
    if ($canmark && !empty($sess->preventsharedip)) {
        if ($sess->preventsharedip == BEHAVIOUR_SHAREDIP_MINUTES) {
            ...
            $record = $DB->get_record_select('behaviour_log', $sql, $params);
        } else {
            // Assume BEHAVIOUR_SHAREDIP_FORCED.
        ...
        if (!empty($record)) {
            ...
            if ($log) {
                ...
                $behaviourid = $DB->get_field('behaviour_sessions', 'behaviourid', array('id' => $record->sessionid));
                $cm = get_coursemodule_from_instance('behaviour', $behaviourid);
                $event = \mod_behaviour\event\session_ip_shared::create(array(
                    'objectid' => 0,
                    'context' => \context_module::instance($cm->id),
                    'other' => array(
...
/**
 * Generate worksheet for behaviour export
 *
 ...
 *
 */
function behaviour_exporttotableed($data, $filename, $format) {
    global $CFG;
    ...
    $myxls = $workbook->add_worksheet(get_string('modulenameplural', 'behaviour'));
    // Format types.
...
/**
 * Generate csv for behaviour export
 *
 * @param stdclass $data The data for the report
 * @param string $filename The name of the file
 *
 */
function behaviour_exporttocsv($data, $filename) {
    $filename .= ".txt";
 ...
 * @param stdClass $formdata moodleform - behaviour form.
 * @param mod_behaviour_structure $att - used to get behaviour level subnet.
 * @return array.
 */
function behaviour_construct_sessions_data_for_add($formdata, mod_behaviour_structure $att) {
    global $CFG;
    ...
    if (empty(get_config('behaviour', 'enablewarnings'))) {
        $absenteereport = get_config('behaviour', 'absenteereport_default');
    } else {
    ...
    if (empty(get_config('behaviour', 'studentscanmark'))) {
    ...
    if (isset($formdata->addmultiply)) {
        ...
        while ($sdate < $enddate) {
            if ($sdate < strtotime('+1 week', $startweek)) {
                ...
                if (isset($formdata->sdays) && array_key_exists($wdaydesc[$dinfo['wday']], $formdata->sdays)) {
                    ...
                    if (isset($formdata->studentscanmark)) { // Students will be able to mark their own behaviour.
                        ...
                        if (!empty($formdata->randompassword)) {
                            $sess->studentpassword = behaviour_random_string();
                        ...
                        if (!empty($formdata->rotateqrcode)) {
                            ...
                            $sess->studentpassword = behaviour_random_string();
                            $sess->rotateqrcodesecret = behaviour_random_string();
                        }
                        if (!empty($formdata->preventsharedip)) {
                            $sess->preventsharedip = $formdata->preventsharedip;
                        }
                    ...
                    behaviour_fill_groupid($formdata, $sessions, $sess);
                }
        ...
        if (isset($formdata->studentscanmark) && !empty($formdata->studentscanmark)) {
            // Students will be able to mark their own behaviour.
            $sess->studentscanmark = 1;
...
/**
 * Helper function for behaviour_construct_sessions_data_for_add().
 *
 * @param stdClass $formdata
 * @param stdClass $sessions
 * @param stdClass $sess
 */
function behaviour_fill_groupid($formdata, &$sessions, $sess) {
    if ($formdata->sessiontype == mod_behaviour_structure::SESSION_COMMON) {
        $sess = clone $sess;
        $sess->groupid = 0;
        $sessions[] = $sess;
    } else {
 ...
 * @return stdClass
 */
function behaviour_course_users_points($courseids = array(), $orderby = '') {
    global $DB;
...
SELECT a.id, a.course as courseid, c.fullname as coursename, atl.studentid AS userid, COUNT(DISTINCT ats.id) AS numtakensessions,
                   ...
                   FROM {behaviour_sessions} ats
                   JOIN {behaviour} a ON a.id = ats.behaviourid
                   JOIN {course} c ON c.id = a.course
                   JOIN {behaviour_log} atl ON (atl.sessionid = ats.id)
                   JOIN {behaviour_statuses} stg ON (stg.id = atl.statusid AND stg.deleted = 0 AND stg.visible = 1)
                   JOIN (SELECT behaviourid, setnumber, MAX(grade) AS maxgrade
                           FROM {behaviour_statuses}
                          WHERE deleted = 0
                            AND visible = 1
                         GROUP BY behaviourid, setnumber) stm
                     ON (stm.setnumber = ats.statusset AND stm.behaviourid = ats.behaviourid)
                  {$joingroup}
 ...
 * @return stdClass
 */
function behaviour_get_users_to_notify($courseids = array(), $orderby = '', $allfornotify = false) {
    global $DB, $CFG;
    ...
    $sql = "SELECT {$idfield} as uniqueid, a.id as aid, {$unames2} a.name as aname, cm.id as cmid, c.id as courseid,
                   ...
                   JOIN {modules} md ON md.id = cm.module AND md.name = 'behaviour'
                   JOIN {behaviour_log} atl ON (atl.sessionid = ats.id)
                   JOIN {user} u ON (u.id = atl.studentid)
                   JOIN {behaviour_statuses} stg ON (stg.id = atl.statusid AND stg.deleted = 0 AND stg.visible = 1)
                   JOIN {behaviour_warning} n ON n.idnumber = a.id
                   LEFT JOIN {behaviour_warning_done} ns ON ns.notifyid = n.id AND ns.userid = atl.studentid
                   JOIN (SELECT behaviourid, setnumber, MAX(grade) AS maxgrade
                           FROM {behaviour_statuses}
                          WHERE deleted = 0
                            AND visible = 1
                     ...
                     ON (stm.setnumber = ats.statusset AND stm.behaviourid = ats.behaviourid)
                  {$joingroup}
                  WHERE ats.absenteereport = 1 AND ats.behaviourid IN (SELECT distinct behaviourid
                                                                          FROM {behaviour_sessions}
    ...
    if (!$allfornotify) {
        ...
        // Only show one record per behaviour for teacher reports.
 ...
 */
function behaviour_template_variables($record) {
    $templatevars = array(
 ...
 *
 * @param mod_behaviour_structure $att behaviour structure
 * @param stdclass $attforsession behaviour_session record.
 * @param int $scantime - time that session should be recorded against.
 * @return bool/int
 */
function behaviour_session_get_highest_status(mod_behaviour_structure $att, $attforsession, $scantime = null) {
 ...
 * @return array
 */
function behaviour_get_automarkoptions() {

    $options = array();

    $options[BEHAVIOUR_AUTOMARK_DISABLED] = get_string('noautomark', 'behaviour');
    if (strpos(get_config('tool_log', 'enabled_stores'), 'logstore_standard') !== false) {
        $options[BEHAVIOUR_AUTOMARK_ALL] = get_string('automarkall', 'behaviour');
    }
    $options[BEHAVIOUR_AUTOMARK_CLOSE] = get_string('automarkclose', 'behaviour');
    $options[BEHAVIOUR_AUTOMARK_ACTIVITYCOMPLETION] = get_string('onactivitycompletion', 'behaviour');
 ...
 */
function behaviour_get_coursemodulenames($id) {
 ...
 * @return array
 */
function behaviour_get_sharedipoptions() {
    $options = array();
    $options[BEHAVIOUR_SHAREDIP_DISABLED] = get_string('no');
    $options[BEHAVIOUR_SHAREDIP_FORCE] = get_string('yes');
    $options[BEHAVIOUR_SHAREDIP_MINUTES] = get_string('setperiod', 'behaviour');
 ...
 */
function behaviour_strftimehm($time) {
    $mins = userdate($time, '%M');

    if ($mins == '00') {
        $format = get_string('strftimeh', 'behaviour');
    } else {
        $format = get_string('strftimehm', 'behaviour');
    }
 ...
 */
function behaviour_construct_session_time($datetime, $duration) {
    $starttime = behaviour_strftimehm($datetime);
    $endtime = behaviour_strftimehm($datetime + $duration);
...
function mod_behaviour_construct_session_full_date_time($datetime, $duration) {
    $sessinfo = userdate($datetime, get_string('strftimedmyw', 'behaviour'));
    $sessinfo .= ' '.behaviour_construct_session_time($datetime, $duration);
 ...
 */
function behaviour_renderqrcode($session) {
    global $CFG;

    if (strlen($session->studentpassword) > 0) {
        $qrcodeurl = $CFG->wwwroot . '/mod/behaviour/behaviour.php?qrpass=' .
            $session->studentpassword . '&sessid=' . $session->id;
    } else {
        $qrcodeurl = $CFG->wwwroot . '/mod/behaviour/behaviour.php?sessid=' . $session->id;
    }

    $barcode = new TCPDF2DBarcode($qrcodeurl, 'QRCODE');
    $image = $barcode->getBarcodePngData(15, 15);
    echo html_writer::img('data:image/png;base64,' . base64_encode($image), get_string('qrcode', 'behaviour'));
}
 ...
 * @param stdClass $session
 */
function behaviour_generate_passwords($session) {
    global $DB;
    $attconfig = get_config('behaviour');
    $password = array();

    for ($i = 0; $i < 30; $i++) {
        array_push($password, array("behaviourid" => $session->id,
            "password" => random_string(), "expirytime" => time() + ($attconfig->rotateqrcodeinterval * $i)));
    }

    $DB->insert_records('behaviour_rotate_passwords', $password);
}
 ...
 * @param stdClass $session
 */
function behaviour_renderqrcoderotate($session) {
    // Load required js.
    ...
    echo html_writer::tag('script', '',
        [
            'src' => 'js/password/behaviour_QRCodeRotate.js',
            'type' => 'text/javascript'
        ]
    );
    echo html_writer::div('', '', ['id' => 'qrcode']); // Div to display qr code.
    echo html_writer::div(get_string('qrcodevalidbefore', 'behaviour').' '.
                          html_writer::span('0', '', ['id' => 'rotate-time']).' '
                          .get_string('qrcodevalidafter', 'behaviour'), 'qrcodevalid'); // Div to display timer.
    // Js to start the password manager.
    echo '
    <script type="text/javascript">
        let qrCodeRotate = new behaviour_QRCodeRotate();
 ...
 * @param stdClass $session
 */
function behaviour_return_passwords($session) {
    global $DB;

    $sql = 'SELECT * FROM {behaviour_rotate_passwords} WHERE behaviourid = ? AND expirytime > ? ORDER BY expirytime ASC';
    return json_encode($DB->get_records_sql($sql, ['behaviourid' => $session->id, time()], $strictness = IGNORE_MISSING));
}
