...
if ($course->id == SITEID) {
    ...
    $strtitle = get_string('coursemessage', 'mod_behaviour');
...
if ($count) {
    if ($count == 1) {
        $heading = get_string('addedrecip', 'mod_behaviour', $count);
    } else {
        $heading = get_string('addedrecips', 'mod_behaviour', $count);
    }
...
';
            echo "<h3>".get_string('previewhtml', 'mod_behaviour')."</h3>";
            ...
            echo '<p align="center"><a href="index.php?id='.$id.'">'.get_string('backtoparticipants', 'mod_behaviour').'</a></p>';
        }
        echo $OUTPUT->footer();
        exit;
    } else {
...
echo '<p align="center"><a href="'.$returnto.'">'.get_string("keepsearching", 'mod_behaviour').'</a>'.
    ((count($SESSION->emailto[$id])) ? ', '.get_string('usemessageform', 'mod_behaviour') : '').'</p>';
