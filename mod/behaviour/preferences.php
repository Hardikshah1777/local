...
$pageparams = new mod_behaviour_preferences_page_params();
...
$maxstatusset = behaviour_get_max_statusset($att->id);
...
$PAGE->set_title($course->shortname. ": ".$att->name.' - '.get_string('settings', 'behaviour'));
$PAGE->set_heading($course->fullname);
$PAGE->force_settings_menu(true);
$PAGE->set_cacheable(true);
$PAGE->navbar->add(get_string('settings', 'behaviour'));
...
switch ($att->pageparams->action) {
    case mod_behaviour_preferences_page_params::ACTION_ADD:
        ...
        $status = behaviour_add_status($newstatus);
        if (!$status) {
            $notification = $OUTPUT->notification(get_string('cantaddstatus', 'behaviour'), 'error');
        }
        ...
        break;
    case mod_behaviour_preferences_page_params::ACTION_DELETE:
        if (behaviour_has_logs_for_status($att->pageparams->statusid)) {
            throw new moodle_exception('cantdeletestatus', 'behaviour', "attsettings.php?id=$id");
        }
        ...
        if (isset($confirm)) {
            behaviour_remove_status($status);
            redirect($att->url_preferences(), get_string('statusdeleted', 'behaviour'));
        }

        $message = get_string('deletecheckfull', 'behaviour', get_string('variable', 'behaviour'));
        $message .= str_repeat(html_writer::empty_tag('br'), 2);
        $message .= $status->acronym.': '.
                    ($status->description ? $status->description : get_string('nodescription', 'behaviour'));
        ...
        echo $OUTPUT->footer();
        exit;
    case mod_behaviour_preferences_page_params::ACTION_HIDE:
        $statuses = $att->get_statuses(false);
        $status = $statuses[$att->pageparams->statusid];
        // Prevent hiding status if in-use.
        if (behaviour_has_logs_for_status($att->pageparams->statusid)) {
            throw new moodle_exception('canthidestatus', 'behaviour', "attsettings.php?id=$id");
        }
        behaviour_update_status($status, null, null, null, 0, $att->context, $att->cm);
        break;
    case mod_behaviour_preferences_page_params::ACTION_SHOW:
        $statuses = $att->get_statuses(false);
        $status = $statuses[$att->pageparams->statusid];
        behaviour_update_status($status, null, null, null, 1, $att->context, $att->cm);
        break;
    case mod_behaviour_preferences_page_params::ACTION_SAVE:
        ...
        foreach ($acronym as $id => $v) {
            ...
            $errors[$id] = behaviour_update_status($status, $acronym[$id], $description[$id], $grade[$id],
...
$prefdata = new mod_behaviour\output\preferences_data($att, array_filter($errors));
$setselector = new mod_behaviour\output\set_selector($att, $maxstatusset);
...
echo $OUTPUT->box(get_string('preferences_desc', 'behaviour'), 'generalbox behaviourdesc', 'notice');
