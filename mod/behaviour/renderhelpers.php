...
/**
 * behaviour module renderering helpers
 *
 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class behaviour_user_sessions_cells_generator {
    ...
    /**
     * Set up params.
     * @param mod_behaviour\output\report_data $reportdata - reportdata.
     * @param stdClass $user - user record.
     */
    public function  __construct(mod_behaviour\output\report_data $reportdata, $user) {
    ...
    public function get_cells($remarks = false) {
        foreach ($this->reportdata->sessions as $sess) {
            ...
            !empty($this->reportdata->sessionslog[$this->user->id][$sess->id]->statusid)) {
                ...
                foreach (explode(',',$statusid) as $sid) {
                    if (array_key_exists( $sid, $this->reportdata->statuses )) {
                        if ($this->reportdata->statuses[$sid] == $this->reportdata->statuses[$sid]) {
                            $points = format_float( $this->reportdata->statuses[$sid]->grade, 1, true, true );
                            $maxpoints = format_float( $sess->maxpoints, 1, true, true );
                            $this->construct_existing_status_cell( $this->reportdata->statuses[$sid]->acronym ." ({$points}/{$maxpoints})" );
                        }
                    } else {
                        if (!empty( $this->reportdata->allstatuses[$sid] &&
                            isset( $this->reportdata->allstatuses[$sid]->acronym ) )) {
                            $statusac = $this->reportdata->allstatuses[$sid]->acronym;
                        } else {
                            $statusac = get_string( 'unknownstatus', 'mod_behaviour', $sid );
                        }
                ...
                if ($this->user->enrolmentstart > ($sess->sessdate + $sess->duration)) {
                    $starttext = get_string('enrolmentstart', 'behaviour', userdate($this->user->enrolmentstart, '%d.%m.%Y'));
                    $this->construct_enrolments_info_cell($starttext);
                } else if ($this->user->enrolmentend && $this->user->enrolmentend < $sess->sessdate) {
                    $endtext = get_string('enrolmentend', 'behaviour', userdate($this->user->enrolmentend, '%d.%m.%Y'));
                    ...
                    $suspendext = get_string('enrolmentsuspended', 'behaviour', userdate($this->user->enrolmentend, '%d.%m.%Y'));
 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class behaviour_user_sessions_cells_html_generator extends behaviour_user_sessions_cells_generator {
    ...
    protected function construct_existing_status_cell($text) {
        ...
        $this->cells[] = html_writer::span($text, 'behaviourstatus-'.$text);
    }
 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class behaviour_user_sessions_cells_text_generator extends behaviour_user_sessions_cells_generator {
 ...
 * @return string.
 */
function behaviour_construct_user_data_stat($usersummary, $view) {
    $stattable = new html_table();
    $stattable->attributes['class'] = 'attlist';
    $row = new html_table_row();
    $row->attributes['class'] = 'normal';
    $row->cells[] = get_string('sessionscompleted', 'behaviour') . ':';
    $row->cells[] = $usersummary->numtakensessions;
    $stattable->data[] = $row;

    $row = new html_table_row();
    $row->attributes['class'] = 'normal';
    $row->cells[] = get_string('pointssessionscompleted', 'behaviour') . ':';
    $row->cells[] = $usersummary->pointssessionscompleted;
    $stattable->data[] = $row;

    $row = new html_table_row();
    $row->attributes['class'] = 'normal';
    $row->cells[] = get_string('percentagesessionscompleted', 'behaviour') . ':';
    ...
    if ($view == ATT_VIEW_ALL) {
        ...
        $row->cells[] = get_string('sessionstotal', 'behaviour') . ':';
        $row->cells[] = $usersummary->numallsessions;
        $stattable->data[] = $row;

        $row = new html_table_row();
        $row->attributes['class'] = 'highlight';
        $row->cells[] = get_string('pointsallsessions', 'behaviour') . ':';
        $row->cells[] = $usersummary->pointsallsessions;
        $stattable->data[] = $row;

        $row = new html_table_row();
        $row->attributes['class'] = 'highlight';
        $row->cells[] = get_string('percentageallsessions', 'behaviour') . ':';
        $row->cells[] = $usersummary->allsessionspercentage;
        $stattable->data[] = $row;

        $row = new html_table_row();
        $row->attributes['class'] = 'normal';
        $row->cells[] = get_string('maxpossiblepoints', 'behaviour') . ':';
        $row->cells[] = $usersummary->maxpossiblepoints;
        $stattable->data[] = $row;

        $row = new html_table_row();
        $row->attributes['class'] = 'normal';
        $row->cells[] = get_string('maxpossiblepercentage', 'behaviour') . ':';
 ...
 *
 * @param stdclass $behaviour - behaviour record.
 * @param stdclass $user - user record
 * @return string.
 *
 */
function behaviour_construct_full_user_stat_html_table($behaviour, $user) {
    $summary = new mod_behaviour_summary($behaviour->id, $user->id);
    return behaviour_construct_user_data_stat($summary->get_all_sessions_summary_for($user->id), ATT_VIEW_ALL);
}
