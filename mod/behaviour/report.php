...
$pageparams = new mod_behaviour_report_page_params();
...
$attrecord = $DB->get_record('behaviour', array('id' => $cm->instance), '*', MUST_EXIST);
...
$att = new mod_behaviour_structure($attrecord, $cm, $course, $context, $pageparams);

$PAGE->set_url($att->url_report());
$PAGE->set_pagelayout('report');
$PAGE->set_title($course->shortname. ": ".$att->name.' - '.get_string('report', 'behaviour'));
$PAGE->set_heading($course->fullname);
$PAGE->force_settings_menu(true);
$PAGE->set_cacheable(true);
$PAGE->navbar->add(get_string('report', 'behaviour'));

$output = $PAGE->get_renderer('mod_behaviour');
$filtercontrols = new mod_behaviour\output\filter_controls($att, true);
$reportdata = new mod_behaviour\output\report_data($att);

// Trigger a report viewed event.
$event = \mod_behaviour\event\report_viewed::create(array(
    'objectid' => $att->id,
    'context' => $PAGE->context,
    'other' => array()
));
$event->add_record_snapshot('course_modules', $cm);
$event->add_record_snapshot('behaviour', $attrecord);
$event->trigger();
