...
if (optional_param('deletehiddensessions', false, PARAM_TEXT)) {
    $pageparams->action = mod_behaviour_sessions_page_params::ACTION_DELETE_HIDDEN;
}
...
if (empty($pageparams->action)) {
    ...
    $url = new moodle_url('/mod/behaviour/view.php', array('id' => $id));
    redirect($url, get_string('invalidaction', 'mod_behaviour'), 2);
}
...
switch ($att->pageparams->action) {
    case mod_behaviour_sessions_page_params::ACTION_ADD:
        $url = $att->url_sessions(array('action' => mod_behaviour_sessions_page_params::ACTION_ADD));
        $mform = new \mod_behaviour\form\addsession($url, $formparams);
        ...
        if ($formdata = $mform->get_data()) {
            $sessions = behaviour_construct_sessions_data_for_add($formdata, $att);
            $att->add_sessions($sessions);
            $att->save_customfields($sessions, $formdata);

            if (count($sessions) == 1) {
                $message = get_string('sessiongenerated', 'behaviour');
            } else {
                $message = get_string('sessionsgenerated', 'behaviour', count($sessions));
            }
        ...
        break;
    case mod_behaviour_sessions_page_params::ACTION_UPDATE:
        $sessionid = required_param('sessionid', PARAM_INT);

        $url = $att->url_sessions(array('action' => mod_behaviour_sessions_page_params::ACTION_UPDATE, 'sessionid' => $sessionid));
        $formparams['sessionid'] = $sessionid;
        $mform = new \mod_behaviour\form\updatesession($url, $formparams);
        ...
        if ($formdata = $mform->get_data()) {
            ...
            mod_behaviour_notifyqueue::notify_success(get_string('sessionupdated', 'behaviour'));
            redirect($att->url_manage());
        }
        break;
    case mod_behaviour_sessions_page_params::ACTION_DELETE:
        $sessionid = required_param('sessionid', PARAM_INT);
        $confirm   = optional_param('confirm', null, PARAM_INT);

        if (isset($confirm) && confirm_sesskey()) {
            $att->delete_sessions(array($sessionid));
            behaviour_update_users_grade($att);
            redirect($att->url_manage(), get_string('sessiondeleted', 'behaviour'));
        }

        $sessinfo = $att->get_session_info($sessionid);

        $message = get_string('deletecheckfull', 'behaviour', get_string('session', 'behaviour'));
        $message .= str_repeat(html_writer::empty_tag('br'), 2);
        $message .= userdate($sessinfo->sessdate, get_string('strftimedmyhm', 'behaviour'));
        ...
        echo $OUTPUT->footer();
        exit;
    case mod_behaviour_sessions_page_params::ACTION_DELETE_SELECTED:
        $confirm    = optional_param('confirm', null, PARAM_INT);
        $message = get_string('deletecheckfull', 'behaviour', get_string('sessions', 'behaviour'));

        if (isset($confirm) && confirm_sesskey()) {
            $sessionsids = required_param('sessionsids', PARAM_ALPHANUMEXT);
            $sessionsids = explode('_', $sessionsids);
            if ($att->pageparams->action == mod_behaviour_sessions_page_params::ACTION_DELETE_SELECTED) {
        ...
        if (empty($sessid)) {
            throw new moodle_exception('nosessionsselected', 'mod_behaviour', $att->url_manage());
        }
        ...
        echo $OUTPUT->footer();
        exit;
    case mod_behaviour_sessions_page_params::ACTION_CHANGE_DURATION:
        $sessid = optional_param_array('sessid', '', PARAM_SEQUENCE);
        $ids = optional_param('ids', '', PARAM_ALPHANUMEXT);

        $slist = !empty($sessid) ? implode('_', $sessid) : '';

        $url = $att->url_sessions(array('action' => mod_behaviour_sessions_page_params::ACTION_CHANGE_DURATION));
        $formparams['ids'] = $slist;
        $mform = new mod_behaviour\form\duration($url, $formparams);
        ...
        if ($formdata = $mform->get_data()) {
            ...
            redirect($att->url_manage(), get_string('sessionupdated', 'behaviour'));
        }
        ...
        break;
    case mod_behaviour_sessions_page_params::ACTION_DELETE_HIDDEN:
        $confirm  = optional_param('confirm', null, PARAM_INT);
        if ($confirm && confirm_sesskey()) {
            $sessions = $att->get_hidden_sessions();
            $att->delete_sessions(array_keys($sessions));
            redirect($att->url_manage(), get_string('hiddensessionsdeleted', 'behaviour'));
        }

        $a = new stdClass();
        $a->count = $att->get_hidden_sessions_count();
        $a->date = userdate($course->startdate);
        $message = get_string('confirmdeletehiddensessions', 'behaviour', $a);
