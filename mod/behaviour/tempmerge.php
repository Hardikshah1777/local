...
$PAGE->set_title($course->shortname.": ".$att->name.' - '.get_string('tempusermerge', 'behaviour'));
$PAGE->set_heading($course->fullname);
$PAGE->set_cacheable(true);
$PAGE->navbar->add(get_string('tempusermerge', 'behaviour'));
...
$mform = new mod_behaviour\form\tempmerge(null, $custom);
...
if ($mform->is_cancelled()) {
    ...
    $sql = "SELECT s.id, lr.id AS reallogid, lt.id AS templogid
              ...
              LEFT JOIN {behaviour_log} lr ON lr.sessionid = s.id AND lr.studentid = :realuserid
              LEFT JOIN {behaviour_log} lt ON lt.sessionid = s.id AND lt.studentid = :tempuserid
             WHERE s.behaviourid = :behaviourid AND lt.id IS NOT NULL
             ORDER BY s.id";
    $params = array(
    ...
    foreach ($logs as $log) {
        if (!is_null($log->reallogid)) {
            // Remove the existing behaviour for the real user for this session.
            $DB->delete_records('behaviour_log', array('id' => $log->reallogid));
        }
        // Adjust the 'temp user' behaviour record to point at the real user.
        $DB->set_field('behaviour_log', 'studentid', $data->participant, array('id' => $log->templogid));
    }
...
echo $output->header();
echo $output->heading(get_string('tempusermerge', 'behaviour').' : '.format_string($course->fullname));
$mform->display();
