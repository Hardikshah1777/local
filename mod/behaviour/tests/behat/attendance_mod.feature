@mod @mod_behaviour
Feature: Teachers and Students can record session behaviour
  In order to record session behaviour
  As a student
  I need to be able to mark my own behaviour to a session
  And as a teacher
  I need to be able to mark any students behaviour to a session
  In order to report on session behaviour
  As a teacher
  I need to be able to export session behaviour and run reports
  In order to contact students with poor behaviour
  As a teacher
  I need the ability to message a group of students with low behaviour
  ...
  Background:
    ...
    And the following "activities" exist:
      ...
      | behaviour | behaviour | C1     |

  @javascript
  Scenario: Students can mark their own behaviour and teacher can hide specific status from students.
    Given I am on the "behaviour" "mod_behaviour > View" page logged in as "teacher1"
    And I click on "Add session" "button"
    And I set the field "Allow students to record own behaviour" to "1"
    ...
    And I log out
    And I am on the "behaviour" "mod_behaviour > View" page logged in as "student1"
    And I follow "Submit behaviour"
    And I should see "Excused"
    And I log out
    And I am on the "behaviour" "mod_behaviour > View" page logged in as "teacher1"
    ...
    Then "behaviour taken by student" "link" should exist
  ...
  Scenario: Teachers can view below % report and send a message
    ...
    And I click on "id_submitbutton" "button"
    And I am on the "behaviour" "mod_behaviour > Report" page
    And I follow "Below"
    ...
    And I click on "Logs" "link"
    And I click on "Get these logs" "button"
    Then "behaviour report viewed" "link" should exist
  ...
  Scenario: Test enabling custom user profile field
    ...
    And I navigate to "Plugins > Activity modules > behaviour" in site administration
  ...
  Scenario: Test adding custom user profile
    ...
    And the following config values are set as admin:
    | customexportfields | superfield | behaviour |
