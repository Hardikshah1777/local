...
$url = new moodle_url('/mod/behaviour/warnings.php');

// This page is used for configuring default set and for configuring behaviour level set.
if (empty($id)) {
    ...
    echo $OUTPUT->header();
    echo $OUTPUT->heading(get_string('defaultwarnings', 'mod_behaviour'));
    $tabmenu = behaviour_print_settings_tabs('defaultwarnings');
    echo $tabmenu;

} else {
    // This is an behaviour level config.
    ...
    require_capability('mod/behaviour:changepreferences', $context);

    $att = new mod_behaviour_structure($att, $cm, $course, $PAGE->context);
...
$mform = new mod_behaviour\form\addwarning($url, array('notid' => $notid, 'id' => $id));
...
if ($data = $mform->get_data()) {
    if (empty($data->notid)) {
        ...
        $existingrecord = $DB->record_exists('behaviour_warning', array('idnumber' => $notify->idnumber,
                                                                         'warningpercent' => $notify->warningpercent,
                                                                              'warnafter' => $notify->warnafter));
        if (empty($existingrecord)) {
            $DB->insert_record('behaviour_warning', $notify);
            echo $OUTPUT->notification(get_string('warningupdated', 'mod_behaviour'), 'success');
        } else {
            echo $OUTPUT->notification(get_string('warningfailed', 'mod_behaviour'), 'warning');
        }

    } else {
        $notify = $DB->get_record('behaviour_warning', array('id' => $data->notid));
        if (!empty($id) && $data->idnumber != $att->id) {
            // Someone is trying to update a record for a different behaviour.
            ...
            $existingrecord = $DB->get_record('behaviour_warning', array('idnumber' => $notify->idnumber,
                'warningpercent' => $notify->warningpercent, 'warnafter' => $notify->warnafter));
            if (empty($existingrecord) || $existingrecord->id == $notify->id) {
                $DB->update_record('behaviour_warning', $notify);
                echo $OUTPUT->notification(get_string('warningupdated', 'mod_behaviour'), 'success');
            } else {
                echo $OUTPUT->notification(get_string('warningfailed', 'mod_behaviour'), 'error');
            }
        }
    }
}
if ($action == 'delete' && !empty($notid)) {
    if (!optional_param('confirm', false, PARAM_BOOL)) {
        ...
        echo $OUTPUT->confirm(get_string('deletewarningconfirm', 'mod_behaviour'), $url, $cancelurl);
        echo $OUTPUT->footer();
        exit;
    } else {
        require_sesskey();
        ...
        $DB->delete_records('behaviour_warning', $params);
        echo $OUTPUT->notification(get_string('warningdeleted', 'mod_behaviour'), 'success');
    }
}
if ($action == 'update' && !empty($notid)) {
    $existing = $DB->get_record('behaviour_warning', array('id' => $notid));
    ...
    if (empty($id)) {
        $warningdesc = get_string('warningdesc', 'mod_behaviour');
        $idnumber = 0;
    } else {
        $warningdesc = get_string('warningdesc_course', 'mod_behaviour');
        $idnumber = $att->id;
    }
    echo $OUTPUT->box($warningdesc, 'generalbox behaviourdesc', 'notice');
    $existingnotifications = $DB->get_records('behaviour_warning',
        array('idnumber' => $idnumber),
        'warningpercent');

    if (!empty($existingnotifications)) {
        $table = new html_table();
        $table->head = array(get_string('warningthreshold', 'mod_behaviour'),
            get_string('numsessions', 'mod_behaviour'),
            get_string('emailsubject', 'mod_behaviour'),
            '');
