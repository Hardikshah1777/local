    ...
    public function execute() {
        ...
        foreach ($customcerts as $customcert) {
            ...
            foreach ($issuedusers as $user) {
				...
				$filecontents_others = $template->generate_pdf_others(false, $user->id, true);
				...
				$tempfile_others = $tempdir . '/' . md5(microtime() . $user->id) . '.pdf';
                file_put_contents($tempfile_others, $filecontents_others);
                ...
                if (!empty($customcert->emailothers)) {
                    ...
                    foreach ($others as $email) {
                        ...
                        if (validate_email($email)) {
                            ...
                            email_to_user($emailuser, fullname($userfrom), $subject, $message, $messagehtml, $tempfile_others,
                                $filename);
                        }
                    }
                }
                $emaildepartmentmanagers = true;
                if ($emaildepartmentmanagers) {
                    global $DB;
                    mtrace('email manager');
                    $companyid = $DB->get_record_sql("SELECT companyid FROM {company_users} WHERE userid = $user->id");
                    if($companyid->companyid) {
                        $managers = $DB->get_records_sql("SELECT usr.* FROM {user} usr 
                                                            JOIN {company_users} cu ON cu.userid = usr.id 
                                                            WHERE cu.managertype IN (1,2) AND usr.deleted = 0 AND usr.suspended = 0 
                                                            AND cu.companyid = $companyid->companyid");
                        foreach ($managers as $manager) {
                                ...
                                $message = $textrenderer->render($renderable);
                                $messagehtml = $htmlrenderer->render($renderable);
                                email_to_user($manager, fullname($userfrom), $subject, $message, $messagehtml, $tempfile_others,
                                    $filename);
