...
$PAGE->set_pagelayout('certificate');
if ($checkallofsite) {
...
if ($code) {
    ...
    $sql = "SELECT ci.id, u.id as userid, $userfields, co.id as courseid,
                   ...
                   c.name as certificatename, c.verifyany, co.summary, u.picture, ci.code,u.email,u.country
              FROM {customcert} c
    ...
    if (!$result->success) {
        $sql = "SELECT ci.id, u.id as userid, $userfields, co.id as courseid,
                   ...
                   c.name as certificatename, c.verifyany, co.summary, u.picture, ci.code,u.email,u.country
              FROM {customcert} c
              JOIN {local_recompletion_ci} ci
                ON c.id = ci.customcertid
              JOIN {course} co
                ON c.course = co.id
              JOIN {user} u
                ON ci.userid = u.id
             WHERE ci.code = :code AND 
             NOT EXISTS (SELECT 1 FROM {local_recompletion_ci} ci2 
             WHERE ci2.userid = ci.userid AND ci2.customcertid = ci.customcertid AND ci2.id > ci.id)";
        if ($issues = $DB->get_records_sql($sql, $params)) {
            $result->success = true;
            $result->issues = $issues;
        }
