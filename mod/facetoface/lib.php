...
function facetoface_get_manageremail($userid) {
    global $DB;
    $mentors = $DB->get_record( 'block_fn_mentor_report_pvt', ['userid' => $userid], 'mentors');
    $data = explode( ',', $mentors->mentors);
    $mentorsusers = $DB->get_record( 'user', ['id' => $data[0]], 'email');
    if (!empty($mentorsusers)){
        return $mentorsusers->email;
    }else{
