    ...
    public function validation($data, $files)
    {
        ...
        if ($data['email']){
            ...
            if (!validate_email($data['email']) || empty($invalidemail) || !empty($user->suspended)) {
                ...
                [$course, $cm] = get_course_and_cm_from_cmid($data['id']);
                ...
                if (empty($cm->available) || empty($enrolled)) {
                    $errors['email'] = get_string('noaccesstomodule1', 'feedback');
                }
            }
...
if (!empty($formdata)){
    ...
    if (!empty($user)){
        ...
        if (!empty($cm->available)) {
            ...
            redirect(new moodle_url('/mod/feedback/complete-verify.php', ['id' => $id]));
        }
    }
}
echo $OUTPUT->header();
...
?>
<style>
    body#page-mod-feedback-verify nav.navbar {
        display: none;
    }
