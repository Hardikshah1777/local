...
/**
 * Define the complete meltassessment structure for backup, with file and id annotations
 */
class backup_meltassessment_activity_structure_step extends backup_activity_structure_step {
    ...
    protected function define_structure() {
        ...
        $evaluation = new backup_nested_element('meltassessment', array('id'), array(
        ...
        // Define sources
        $evaluation->set_source_table('meltassessment', array('id' => backup::VAR_ACTIVITYID));

        // All the rest of elements only happen if we are including user info
        $level->set_source_table('meltassessment_market', array('meltassessmentid' => backup::VAR_PARENTID));

        $section->set_source_table('meltassessment_section', array('meltassessmentid' => backup::VAR_PARENTID));

        $skill->set_source_table('meltassessment_skill', array('sectionid' => backup::VAR_PARENTID));

        $userinfo->set_source_table('meltassessment_field', array('meltassessmentid' => backup::VAR_PARENTID));

        if ($userinfoincluded) {

            $user->set_source_table('meltassessment_user', array('meltassessmentid' => backup::VAR_PARENTID));

            $userskilllevel->set_source_table('meltassessment_user_skill', array('meltassessmentuserid' => backup::VAR_PARENTID));

            $userfieldinfo->set_source_table('meltassessment_field_info', array('meltassessmentuserid' => backup::VAR_PARENTID));

            $userlesson->set_source_table('meltassessment_user_lesson', array('meltassessmentuserid' => backup::VAR_PARENTID));
        ...
        $evaluation->annotate_files('mod_meltassessment', 'intro', null); // This file area hasn't itemid
