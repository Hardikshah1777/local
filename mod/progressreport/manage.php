...
use mod_progressreport\output\sectionlist;
...
$url = new moodle_url('/mod/progressreport/manage.php',['id'=>$id]);
$returnurl = new moodle_url('/course/view.php',['id'=>$cm->course]);
$PAGE->set_context($context);
$PAGE->set_url($url);
$PAGE->set_title(get_string('pluginname','mod_progressreport'));
...
if(!empty($skillid)){
    ...
    $update = $DB->update_record('progressreport_skill',$updatedata);
    redirect($url);
}
...
if(!empty($sectionid)){
    ...
    $delete = $DB->update_record('progressreport_section',$deletedata);
    if($delete){
        ...
        foreach ($skills as $skill){
            ...
            $update = $DB->update_record('progressreport_skill',$deleteskilldata);
        }
    }
    redirect($url);
}
...
if(!empty($id)){
    ...
    $condition = ['progressreportid' => $cm->instance,'deleted' => 0];
    $sectionrecs = $DB->get_records('progressreport_section',$condition, 'sortorder');
    ...
    foreach ($sectionrecs as $sectionrec){
        ...
        $skills = $DB->get_records('progressreport_skill',['sectionid' => $sectionrec->id,'deleted' => 0]);
...
$PAGE->requires->js_call_amd('mod_progressreport/sectionmove', 'init',['skillsectionwrapper', 'move_updown_field.class']);
echo $OUTPUT->footer();