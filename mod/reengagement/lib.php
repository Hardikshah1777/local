...
function reengagement_add_instance($reengagement) {
    ...
    if (empty($reengagement->suppressemail)) {
        ...
        $reengagement->suppresstarget = implode(',', $reengagement->suppresstarget);
    }
...
function reengagement_email_user($reengagement, $inprogress) {
    ...
    if (!empty($reengagement->suppresstarget)) {
        $targetcomplete = reengagement_check_target_completion($user->id, $reengagement->suppresstarget, $inprogress->id);
 ...
 */
function reengagement_check_target_completion($userid, $targetcmid, $inprogressid)
{
    global $DB;
    ...
    $data = explode(',', $targetcmid, PHP_INT_MAX);

    foreach ($data as $row) {
    $conditions = array('userid' => $userid, 'coursemoduleid' => $row);
    ...
    if (!empty($completion) && !empty($rip)) {
        ...
        if ($emailpending) {
            if (empty($reengagement->suppresstarget)) {
                ...
                $inprogress = $rip->id;
                $targetcomplete = reengagement_check_target_completion($USER->id, $cm->id, $inprogress);
                if (!$targetcomplete) {
