...
class mod_scorm_mod_form extends moodleform_mod {
    ...
    public function definition() {
        ...
        $rsoptions = [
            0 => get_string('grade','scorm'),
            1 => get_string('status','scorm'),
        ];
        $mform->addElement('select', 'reportstatus', get_string('reportstatus', 'scorm'), $rsoptions);
        $mform->setType('reportstatus',PARAM_INT);

        $mform->addElement('text','idnumber', get_string('idnumberscorm', 'scorm'),'maxlength="100"  size="10"');
        $mform->setType('idnumber', PARAM_RAW);

        $options = ['optional' => true];
        $mform->addElement('duration','retakeduration', get_string('retakeduration', 'scorm'), $options);
        $mform->setType('retakeduration', PARAM_INT);
