...
define(['jquery', 'core/log', 'core/str', 'core/templates', 'core/notification', 'core/loadingicon', 'core/aria'],
function($, log, str, templates, notification, LoadingIcon, Aria) {
     ...
     * @return {Promise}
     */
    var updateSuggestions = function(options, state, query, originalSelect, showSuggestions = true) {
     ...
     * @return {Promise}
     */
    var updateAjax = function(e, options, state, originalSelect, ajaxHandler, showSuggestions = true) {
        ...
        ajaxHandler.transport(options.selector, query, function(results) {
            ...
            pendingPromise.resolve(updateSuggestions(options, state, '', originalSelect, showSuggestions));
        }, function(error) {
    ...
    return /** @alias module:core/form-autocomplete */ {
        ...
        enhance: function(selector, tags, ajax, placeholder, caseSensitive, showSuggestions, noSelectionString,
            ...
            .then(function(layout, input, suggestions, selection) {
                ...
                const depSelector = formSelect?.dataset.target;
                if (depSelector) {
                    const dependOn = document.querySelector(depSelector);
                    if (dependOn) {
                        dependOn.addEventListener('change', e => {
                            formSelect.selectedIndex = -1;
                            originalSelect.children('option').prop('selected', false);
                            if (options.ajax) {
                                e.currentTarget = input;
                                require([options.ajax], function(ajaxHandler) {
                                    updateAjax(e, options, state, originalSelect, ajaxHandler, false);
