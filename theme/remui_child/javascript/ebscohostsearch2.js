...
function ebscoHostSearchGo(form,id) {
	function ebscoAddDisciplines() {
		function addScrollBoxLimiters(disciplinesDiv) {
			var visibleDisciplines = document.getElementById('ehostVisibleDisciplines' + id),
			...
			function addDisciplinesToUrl () {
				...
				var hiddenDisciplines = disciplinesDiv.getElementById('ehostHiddenDisciplines' + id).getElementsByTagName('option');
		...
		var disciplinesDiv = document.getElementById('disciplineBlock' + id);
		if (disciplinesDiv) {
...
// id + "ebscohostsearchtext" is the variable we're looking for, to see if it exists

	 var ebscohostsearchtext = chkObject(form["ebscohostsearchtext" + id],'');
	 var ebscohostkeywords = chkObject(form.ebscohostkeywords,'');
	 var ebscohostsearchsrc = chkObject(form["ebscohostsearchsrc" + id],'');
	...
	/* [7.5.2017]  Updated below to encodeURIComponent to allow for # wildcard to work in Permalink */
	 ...
	 if (ebscohosturl.indexOf('eds-live') == -1 && ebscohosturl.indexOf('pfi-live') === -1) {
	 		if (ebscohostsearchsrc == "db" || ebscohostsearchsrc == "dbgroup") {
	 	 		var ebscohostdatabases = getSelectedDatabases(form["cbs" + id],ebscohostsearchsrc);
	 ...
	 var cbFT = document.getElementById("chkFullText" + id);
	 ...
	 var cbPR = document.getElementById("chkPeerReviewed" + id);
	 ...
	 var cbCO = document.getElementById("chkCatalogOnly" + id);
	 ...
	 //[a.hazen 7.5.2017] Add IR to EDS
	 	 var cbIRO = document.getElementById("chkIROnly" + id);
	 ...
	 var cbLC = document.getElementById("chkLibraryCollection" + id);
	...
	var keywordSelector = document.getElementById("guidedField_0_" + id);
	var titleSelector = document.getElementById("guidedField_1_" + id);
	var authorSelector = document.getElementById("guidedField_2_" + id);
...
function highlight(id, currentClass, checkboxId, uniqueID) {
 ...
 if(checkbox_element.checked==false)
 		findObj('cball' + uniqueID).checked=false;
}

function SelectAllCheckBoxes(e,uniqueID) {
	var cbs = document.getElementsByName("cbs" + uniqueID),
		rows = document.getElementsByClassName('choose-db-list-' + uniqueID)[0].children;
 	for(var i=0;i<cbs.length;i++){
 		var row = rows[i],
 			checkbox = row.getElementsByTagName('input')[0];
		cbs[i].checked = e.checked;
  		if (row.id) {
  			highlight(row.id, (i % 2) ? 'two' : 'one', checkbox.id, uniqueID);
  		}
 	}
}
