...
class participants extends \table_sql implements dynamic_table {
    ...
    public function query_db($pagesize, $useinitialsbar = true) {
        ...
        if (!empty($SESSION->filters) && !empty($SESSION->filters['user/index'])) {
            $filters = $SESSION->filters['user/index'];
            if (!empty($filters['firstname'])) {
                $conditions[] = $DB->sql_like('firstname', ':firstname', false, false);
                $tparams['firstname'] = '%'.$filters['firstname'].'%';
            }
            if (!empty($filters['lastname'])) {
                $conditions[] = $DB->sql_like('lastname', ':lastname', false, false);
                $tparams['lastname'] = '%'.$filters['lastname'].'%';
            }
        }

        if(!empty($conditions)){
            if ($twhere) {
                $twhere .= ' AND ' . (implode(" AND ", $conditions)) . '';
            } else {
                $twhere .= (implode(" AND ", $conditions));
            }
        }
